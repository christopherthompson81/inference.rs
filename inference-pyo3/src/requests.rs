...
use either::Either;
use inference_core::{AdapterGenerationId, AdapterSelection, ReasoningEffort, WebSearchOptions};
use pyo3::{
...
impl LoraAdapterGeneration {
    pub(crate) fn parse(
        generation: &str,
    ) -> Result<Self, inference_core::AdapterGenerationParseError> {
        Ok(Self {
...
fn parse_reasoning_effort(
    effort: Option<&str>,
) -> Result<Option<ReasoningEffort>, inference_core::ReasoningEffortParseError> {
...
pub struct ChatCompletionRequest {
    ...
    pub(crate) code_execution_permission: Option<inference_core::CodeExecutionPermission>,
    pub(crate) agent_permission: Option<inference_core::AgentPermission>,
...
impl ChatCompletionRequest {
    pub(crate) fn shell_options(&self) -> Option<inference_core::ShellOptions> {
        self.shell_skills
            .as_ref()
            .map(|skills| inference_core::ShellOptions {
                skills: skills.iter().cloned().map(Into::into).collect(),
            })
    }

    pub(crate) fn input_files(&self) -> Vec<inference_core::File> {
        self.input_files
            .clone()
            .unwrap_or_default()
            .into_iter()
