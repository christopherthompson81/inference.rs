...
impl From<NetworkModePy> for inference_core::NetworkMode {
...
impl From<CodeExecutionPermissionPy> for inference_core::CodeExecutionPermission {
...
impl From<AgentPermissionPy> for inference_core::AgentPermission {
...
impl From<&inference_core::AgentToolSource> for AgentToolSourcePy {
    fn from(value: &inference_core::AgentToolSource) -> Self {
        match value {
            inference_core::AgentToolSource::BuiltIn => Self::BuiltIn,
            inference_core::AgentToolSource::User => Self::User,
            inference_core::AgentToolSource::Mcp => Self::Mcp,
            inference_core::AgentToolSource::External => Self::External,
        }
    }
}
...
impl From<&inference_core::AgentToolKind> for AgentToolKindPy {
    fn from(value: &inference_core::AgentToolKind) -> Self {
        match value {
            inference_core::AgentToolKind::CodeExecution => Self::CodeExecution,
            inference_core::AgentToolKind::WebSearch => Self::WebSearch,
            inference_core::AgentToolKind::Shell => Self::Shell,
            inference_core::AgentToolKind::File => Self::File,
            inference_core::AgentToolKind::Custom => Self::Custom,
            inference_core::AgentToolKind::External => Self::External,
        }
    }
}
...
pub struct SandboxPolicy {
    ...
    pub(crate) network: inference_core::NetworkMode,
...
impl SandboxPolicy {
    ...
    fn new(
        ...
        let network = parse_network_mode(network)?.unwrap_or(inference_core::NetworkMode::Loopback);
        Ok(Self {
            max_memory_mb,
            max_cpu_secs,
            max_procs,
...
impl From<SandboxPolicy> for inference_core::SandboxPolicy {
    fn from(p: SandboxPolicy) -> Self {
        inference_core::SandboxPolicy {
...
pub struct CodeExecutionConfig {
    ...
    pub(crate) permission: Option<inference_core::CodeExecutionPermission>,
...
pub struct ShellConfig {
    ...
    pub(crate) working_directory: Option<PathBuf>,
    pub(crate) sandbox_policy: Option<SandboxPolicy>,
    pub(crate) permission: Option<inference_core::AgentPermission>,
}
...
) -> PyResult<Option<inference_core::CodeExecutionPermission>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    PERMISSION_AUTO => Ok(inference_core::CodeExecutionPermission::Auto),
                    PERMISSION_ASK => Ok(inference_core::CodeExecutionPermission::Ask),
                    PERMISSION_DENY => Ok(inference_core::CodeExecutionPermission::Deny),
...
pub(crate) fn parse_agent_permission(
    value: Option<Py<PyAny>>,
) -> PyResult<Option<inference_core::AgentPermission>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                let value = value.bind(py);
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    PERMISSION_AUTO => Ok(inference_core::AgentPermission::Auto),
                    PERMISSION_ASK => Ok(inference_core::AgentPermission::Ask),
                    PERMISSION_DENY => Ok(inference_core::AgentPermission::Deny),
                    other => Err(PyValueError::new_err(format!(
...
fn parse_network_mode(value: Option<Py<PyAny>>) -> PyResult<Option<inference_core::NetworkMode>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                let value = value.bind(py);
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    NETWORK_NONE => Ok(inference_core::NetworkMode::None),
                    NETWORK_LOOPBACK => Ok(inference_core::NetworkMode::Loopback),
                    NETWORK_FULL => Ok(inference_core::NetworkMode::Full),
                    other => Err(PyValueError::new_err(format!(
...
impl From<&inference_core::AgentToolMetadata> for AgentToolMetadataPy {
    fn from(tool: &inference_core::AgentToolMetadata) -> Self {
        Self {
...
impl From<&inference_core::AgentToolApproval> for AgentToolApprovalPy {
    fn from(approval: &inference_core::AgentToolApproval) -> Self {
        Self {
...
impl AgentToolApprovalDecisionPy {
    fn to_core(&self) -> inference_core::AgentToolApprovalDecision {
        match self.decision {
            AgentToolApprovalDecisionKindPy::Approve => {
                inference_core::AgentToolApprovalDecision::approve()
                    .with_remember_for_session(self.remember_for_session)
            }
            AgentToolApprovalDecisionKindPy::Deny => {
                inference_core::AgentToolApprovalDecision::deny(self.message.clone())
            }
        }
    }
}
...
fn parse_agent_approval_result(
    py: Python<'_>,
    result: Py<PyAny>,
) -> PyResult<inference_core::AgentToolApprovalDecision> {
    if let Ok(approve) = result.extract::<bool>(py) {
        return Ok(if approve {
            inference_core::AgentToolApprovalDecision::approve()
        } else {
            inference_core::AgentToolApprovalDecision::deny(None)
        });
    }
...
pub(crate) fn build_agent_approval_callback(
    ...
    callback.map(|callback| {
        Arc::new(move |approval: &inference_core::AgentToolApproval| {
            Python::with_gil(|py| {
                let payload = AgentToolApprovalPy::from(approval);
                callback
                    .call1(py, (payload,))
                    .and_then(|result| parse_agent_approval_result(py, result))
                    .unwrap_or_else(|_| inference_core::AgentToolApprovalDecision::deny(None))
            })
        }) as inference_core::AgentToolApprovalCallback
    })
}

impl From<CodeExecutionConfig> for inference_core::CodeExecutionConfig {
    fn from(cfg: CodeExecutionConfig) -> Self {
        let default = inference_core::CodeExecutionConfig::default();
        let approval_callback = cfg.approval_callback.map(|callback| {
            Arc::new(move |approval: &inference_core::CodeExecutionApproval| {
                Python::with_gil(|py| {
            ...
            }) as inference_core::CodeExecutionApprovalCallback
        });
        inference_core::CodeExecutionConfig {
...
impl From<ShellConfig> for inference_core::ShellConfig {
    fn from(cfg: ShellConfig) -> Self {
        let default = inference_core::ShellConfig::default();
        inference_core::ShellConfig {
...
impl From<ShellSkillMount> for inference_core::ShellSkillMount {
    fn from(skill: ShellSkillMount) -> Self {
        inference_core::ShellSkillMount {
            name: skill.name,
