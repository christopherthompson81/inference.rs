...
const KERNELS: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/inference_paged_attention.metallib"
));
#[cfg(target_os = "ios")]
const KERNELS: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/inference_paged_attention_ios.metallib"
));
#[cfg(target_os = "tvos")]
const KERNELS: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/inference_paged_attention_tvos.metallib"
));
...
impl Kernels {
    ...
    fn compile_kernels_at_runtime(&self, device: &Device) -> Result<Library, MetalKernelError> {
        inference_metal_compile::compile_runtime_library(device, &PAGED_ATTENTION_METAL_SOURCE_SET)
