//! Error types and conversions for the inference SDK.

use thiserror::Error;

/// Error type for the inference SDK.
///
...
pub enum Error {
    ...
    ModelError {
        ...
        partial_response: Option<Box<inference_core::ChatCompletionResponse>>,
    },
    ...
    /// Wraps the existing [`inference_core::InferenceRsError`].
    #[error(transparent)]
    Management(#[from] inference_core::InferenceRsError),
...
impl From<tokio::sync::mpsc::error::SendError<inference_core::Request>> for Error {
    /// Convert from a channel send error (mapped to [`Error::Channel`]).
    fn from(e: tokio::sync::mpsc::error::SendError<inference_core::Request>) -> Self {
        Error::Channel(e.to_string())
    }
}

impl From<Box<inference_core::ResponseErr>> for Error {
    /// Convert from a boxed engine response error.
    fn from(e: Box<inference_core::ResponseErr>) -> Self {
        match *e {
            inference_core::ResponseErr::InternalError(inner) => Error::Inference(inner),
            inference_core::ResponseErr::ValidationError(inner) => {
                Error::RequestValidation(inner.to_string())
            }
            inference_core::ResponseErr::ModelError(message, response) => Error::ModelError {
                message,
                partial_response: Some(Box::new(response)),
            },
            inference_core::ResponseErr::CompletionModelError(message, _) => Error::ModelError {
                message,
