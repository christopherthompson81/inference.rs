...
Agentic sessions hold tool-call records, tool responses, and multimodal payloads from earlier turns. inference stores this state in memory and reconciles it with each new request.
...
- Guide: [persist sessions](/guides/agents/persist-sessions/).
- Reference: [HTTP API `/v1/sessions/{id}`](/reference/http-api/).
