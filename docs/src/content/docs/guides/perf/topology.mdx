...
Topology is a per-layer placement and [ISQ (in-situ quantization)](/reference/quantization-types/) mechanism. A YAML file specifies, per layer range, the device and quantization to use.

Most cases do not need topology. Defaults work for typical hardware; `inference tune` covers common optimization.
...
Topology ISQ pins also apply when producing [UQFF (Universal Quantized File Format)](/reference/uqff-format/) files: pinned layers keep their type in every written variant, with the `--isq` value as the default for the rest.
...
```bash
inference serve --topology topology.yaml -m <model>
```
...
```rust
let model = inference::ModelBuilder::new("<model>")
...
For an introduction to per-layer quantization tradeoffs, see [the quantization guide](/guides/quantization/quantize-a-model/).
