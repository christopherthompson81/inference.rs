...
inference.rs has four ways to spread one model beyond a single GPU. They solve different problems and use different communication paths.
...
| Multi-node NCCL | Multiple machines, each with local CUDA GPUs | NCCL across all ranks, with inference.rs coordinating node startup | You want tensor parallelism across machines and each node contributes one or more local CUDA ranks. |
| Ring backend | Multiple machines | inference.rs ring transport from `RING_CONFIG` | You explicitly want the ring backend instead of NCCL. |
...
2. Multiple visible GPUs use NCCL tensor parallelism when the binary has `cuda nccl` and `INFERENCE_RS_NO_NCCL` is not set.
3. If NCCL is unavailable or disabled, inference.rs uses layer mapping. CUDA pairs use P2P when the driver allows it; otherwise transfers stage through CPU.
4. If `INFERENCE_RS_MN_GLOBAL_WORLD_SIZE` is set, NCCL tensor parallelism is extended across nodes.
5. If `RING_CONFIG` is set and the binary has `ring`, the ring backend is available. If the binary also has NCCL, set `INFERENCE_RS_NO_NCCL=1` to force ring.
...
If NCCL is not installed, omit `nccl`. To force the installer decision, use `INFERENCE_RS_INSTALL_NCCL=1` or `INFERENCE_RS_INSTALL_NO_NCCL=1`. To disable NCCL at runtime without rebuilding:

```bash
INFERENCE_RS_NO_NCCL=1 inference serve -m Qwen/Qwen3-32B --quant 4
```

### Select GPUs

Use `CUDA_VISIBLE_DEVICES` to restrict the GPU set before inference.rs starts:

```bash
CUDA_VISIBLE_DEVICES=0,1 inference serve -m Qwen/Qwen3-32B --quant 4
```
...
If the visible GPU count is incompatible, inference.rs errors instead of selecting a smaller subset. Use `CUDA_VISIBLE_DEVICES` to choose a compatible subset.
...
```bash
inference serve -n "0:32;1:32" -m <model>

# Uneven split
inference serve -n "0:44;1:20" -m Qwen/Qwen3-32B --quant 4
```
...
For per-layer or per-tensor placement, use the [topology guide](/guides/perf/topology/).
...
On Apple Silicon there is no multi-GPU concept: CPU and GPU share unified memory and device mapping is a no-op. `inference doctor` reports a single device on Apple hardware.

## Multi-node NCCL

Multi-node NCCL inference extends tensor parallelism across machines. Each node contributes one or more local CUDA ranks to one global NCCL communicator. It is selected by the `INFERENCE_RS_MN_*` environment variables and does not use `RING_CONFIG`.
...
The global tensor-parallel size must satisfy the same head-divisibility constraints as single-machine TP. Incompatible sizes fail at startup; inference.rs does not automatically drop ranks.

The head node distributes the NCCL id to workers over TCP, then all ranks join one communicator. The variable roles (`INFERENCE_RS_MN_GLOBAL_WORLD_SIZE`, `INFERENCE_RS_MN_LOCAL_WORLD_SIZE`, head vs worker settings) are documented in the [environment variable reference](/reference/environment-variables/). Do not set `INFERENCE_RS_NO_NCCL` or `RING_CONFIG` in this mode.
...
INFERENCE_RS_MN_GLOBAL_WORLD_SIZE=8 \
INFERENCE_RS_MN_LOCAL_WORLD_SIZE=4 \
INFERENCE_RS_MN_HEAD_NUM_WORKERS=1 \
INFERENCE_RS_MN_HEAD_PORT=9000 \
inference serve -m Qwen/Qwen3-32B --quant 4
```
...
INFERENCE_RS_MN_GLOBAL_WORLD_SIZE=8 \
INFERENCE_RS_MN_LOCAL_WORLD_SIZE=4 \
INFERENCE_RS_MN_WORKER_SERVER_ADDR=10.0.0.1:9000 \
INFERENCE_RS_MN_WORKER_ID=0 \
inference serve -m Qwen/Qwen3-32B --quant 4
```
...
```bash
cargo install inference-cli --features "cuda flash-attn ring"
```

If the binary is also built with `nccl`, set `INFERENCE_RS_NO_NCCL=1` when launching so the ring backend is selected.
...
- [Topology](/guides/perf/topology/): per-layer and per-tensor placement on one host.
- [Environment variables](/reference/environment-variables/): full `INFERENCE_RS_MN_*`, `INFERENCE_RS_NO_NCCL`, and `RING_CONFIG` registry.
