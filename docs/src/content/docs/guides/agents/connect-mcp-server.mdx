...
description: Use inference as an MCP client.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

inference.rs can act as a Model Context Protocol (MCP) client, connecting to one or more MCP servers at startup and merging their tools into the model's available set.

MCP tools automatically use [strict tool calling](/guides/agents/tool-calling-basics/#strict-tool-calling) when the MCP server provides an input schema.
...
```bash
inference serve --mcp-config mcp.json -m <model>
```
...
Full schema: [MCP config schema reference](/reference/mcp-config-schema/).
...
runner = inference_rs.Runner(
    which=inference_rs.Which.Plain(model_id="Qwen/Qwen3-4B"),
    mcp_client_config=mcp_config,
)
```

[Full example](/examples/python/mcp-client/)

</TabItem>
<TabItem label="Rust">

```rust
use inference::{McpClientConfig, McpServerConfig, McpServerSource, ModelBuilder};
...
[Full example](/examples/rust/advanced/mcp-client/)
...
MCP tool calls appear in `agentic_tool_calls` records and `agentic_tool_call_progress` streaming events with the prefixed name. They share the [tool-round cap](/guides/agents/tool-calling-basics/#configuring-the-tool-loop).

Full examples: [Python MCP client](/examples/python/mcp-client/), [Rust MCP client](/examples/rust/advanced/mcp-client/), [HTTP chat with MCP tools](/examples/server/mcp-chat/).
