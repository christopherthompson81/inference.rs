...
`--enable-shell` registers a shell execution tool with the model. Commands run in a persistent per-session working directory; on Linux and macOS the CLI wraps the subprocess in the [OS-level sandbox](/reference/sandbox/) by default (`--sandbox auto`).

```bash
inference serve --enable-shell -m <model>
```
...
```bash
inference serve --enable-shell -m Qwen/Qwen3-4B
```
...
```python
from inference_rs import ChatCompletionRequest, Runner, ShellConfig, Which
...
[Full example](/examples/python/shell/)

</TabItem>
<TabItem label="Rust SDK">

```rust
use inference::{ModelBuilder, RequestBuilder, ShellConfig, TextMessageRole, TextMessages};
...
[Full example](/examples/rust/advanced/shell/)
...
- Call the built-in `inference_surface_outputs` tool after a previous shell call created the file.
...
Use `inference_surface_outputs` when the model only knows the final filenames after inspecting command output:

```json
{
  "outputs": [{"name": "inference_artifacts_demo.pptx", "format": "pptx"}]
}
```

Files named in request `files` are merged into shell `outputs` automatically. Files written but not listed in `outputs`, `inference_surface_outputs`, or request `files` remain internal to the shell session.
...
Chat Completions streaming emits the same work as inference.rs `agentic_tool_call_progress` SSE events. The [agentic runtime guide](/guides/agents/agentic-runtime/) covers the event stream and files contract in depth.
...
When sandbox CPU limits are enforced and the configured shell timeout exceeds `max_cpu_secs`, inference.rs raises `max_cpu_secs` to match and logs a warning. This keeps long-running Skills from being capped by a lower CPU limit.

For deployments where shell commands must leave the inference host entirely, use [`--tool-dispatch-url`](/guides/agents/tool-calling-basics/#dispatch-url) and expose a custom executor instead of enabling built-in shell.
...
- Examples: [Python](/examples/python/shell/), [Rust](/examples/rust/advanced/shell/).
