...
```bash
inference serve --enable-search -m <model>
```
...
```python
from inference_rs import ChatCompletionRequest, Runner, Which
...
```rust
use inference::{IsqBits, ModelBuilder, SearchEmbeddingModel};
...
The built-in search and extraction tools use [strict tool calling](/guides/agents/tool-calling-basics/#strict-tool-calling) by default, so generated queries and URLs are constrained to the declared JSON Schema.
...
```bash
inference serve --enable-search \
  --search-embedding-model embedding-gemma \
  -m <model>
```

`--search-embedding-model` accepts `embedding-gemma`. It requires `--enable-search` (or `--agent`/`--agentic`, the [one-flag agent preset](/guides/agents/build-an-agent/) that turns search on).
...
```rust
use std::sync::Arc;
use inference::{ModelBuilder, SearchFunctionParameters, SearchResult};
