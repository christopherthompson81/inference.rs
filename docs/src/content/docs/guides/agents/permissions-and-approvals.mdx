...
`agent_permission` controls whether inference.rs may run an agent action after the model asks for one. It applies to all server-executed actions, not just Python: code execution, shell, web search, file tools, registered callbacks, and external tool dispatch.

```bash
inference run --agent -m google/gemma-4-E4B-it --agent-permission ask
```
...
Permissioning is separate from sandboxing. Permission mode decides whether an action may start; the [sandbox](/reference/sandbox/) controls what generated code and shell commands can access after they start.
...
```bash
inference run --agent -m google/gemma-4-E4B-it --agent-permission deny
```
...
The approval endpoint returns `{"status":"resolved"}`, `{"status":"queued"}`, or `{"status":"not_found"}` (the last with HTTP 404). See the [HTTP API reference](/reference/http-api/) for the exact wire schema and the [HTTP approval example](/examples/server/code-execution-approval/) for a complete client.
...
[Full example](/examples/python/code-execution-approval/)
...
[Full example](/examples/rust/advanced/code-execution-approval/)
