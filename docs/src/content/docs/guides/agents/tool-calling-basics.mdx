---
title: Tool calling
description: OpenAI tool calls, strict schemas, and the inference.rs server-side tool loop.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Tool calling lets the model request external work via a structured invocation. inference.rs supports the standard OpenAI client-side flow plus a server-side loop that runs tools inside one request:
...
**Server-side loop.** inference.rs runs the entire tool loop inside one request: built-in tools (web search, code execution, shell, OpenAI-compatible Skills), [MCP (Model Context Protocol)](/guides/agents/connect-mcp-server/) tools, SDK callbacks, or HTTP dispatch. The client sends one request and receives one final reply. See [how the loop runs](/guides/agents/agentic-runtime/) for the round structure.
...
[Full example](/examples/server/tool-calling/)
...
```python
import json
from inference_rs import ChatCompletionRequest, Runner, ToolChoice, Which
...
Run the tool yourself and send the result back as a `tool` message, or register a matching `tool_callbacks` entry on `Runner` to have inference.rs execute it server-side.

[Full example](/examples/python/tool-call/)
...
[Full example](/examples/rust/advanced/tools/)
...
[Full example](/examples/server/allowed-tools/)
...
[Full example](/examples/rust/advanced/allowed-tools/)
...
With `function.strict: true`, inference.rs constrains the generated arguments to the tool's `parameters` JSON Schema during decoding. Non-strict tools are only parsed as JSON after generation completes, so malformed output can still appear. Use strict mode when malformed or extra arguments would be expensive, unsafe, or annoying to handle in application code.
...
```bash
inference serve --enable-search --enable-code-execution -m <model>
```

`--agent` is shorthand for both flags; see [build an agent](/guides/agents/build-an-agent/). The built-in tools are covered in [web search](/guides/agents/web-search/) and [code execution](/guides/agents/enable-code-execution/).

For custom tools, the cleanest path is to run them as an MCP server and connect inference as a client; see [connect to an MCP server](/guides/agents/connect-mcp-server/). The SDKs can also register custom callbacks directly: Python uses `Runner(tool_callbacks=...)` ([full example](/examples/python/custom-tool-call/)); Rust builders use `with_tool_callback(...)` or `with_tool_callback_and_tool(...)` ([full example](/examples/rust/advanced/tool-callback/)).
...
```bash
inference serve --max-tool-rounds 10 -m <model>
```
...
```bash
inference serve --tool-dispatch-url http://localhost:7070/tools -m <model>
```

Request body sent to the dispatch URL:

```json
{"name": "search", "arguments": {"query": "inference"}}
```

Expected response: a bare string (the tool result) or `{"content": "..."}`. The dispatch URL is server-level only; it cannot be set per-request over HTTP. [Full example](/examples/server/tool-dispatch/)
...
- `agent_permission`: tighten the [permission mode](/guides/agents/permissions-and-approvals/).
...
See the [serve CLI reference](/reference/cli/serve/) for the full flag list.

## Response extensions

When the server-side loop runs, the response includes an `agentic_tool_calls` array (one entry per executed round) and, when streaming, `agentic_tool_call_progress` Server-Sent Events (SSE) around each tool execution. The wire format for both is in the [HTTP API reference](/reference/http-api/).
