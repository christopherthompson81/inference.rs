...
- `--enable-shell`: a shell subprocess that can run commands. It is also the executor used by [OpenAI-compatible Skills](/guides/agents/skills/).

The loop's fallback cap is 256 tool rounds unless `--max-tool-rounds` says otherwise. On Linux and macOS, code and shell execution are [sandboxed by default](/reference/sandbox/) (`--sandbox auto`). The default timeouts are 60 seconds for Python code execution and 600 seconds for shell execution.
...
Apps can make the output contract explicit by declaring files up front. This request asks for a PNG chart and tells inference.rs to surface it as a typed file:
...
The response keeps the normal OpenAI-compatible `choices` array and adds inference.rs fields for tool work, files, and session state. Treat tool identifiers in `agentic_tool_calls` as opaque correlation values; use the `arguments`, `result_content`, and `file_ids` fields for app behavior.
...
`agentic_tool_calls` records the work the server did on behalf of the model. `files` contains structured outputs produced by tools; small files are inlined, larger ones are fetched by id. The wire schema lives in the [HTTP API reference](/reference/http-api/).
...
The [agentic runtime guide](/guides/agents/agentic-runtime/) covers the event stream and files contract in depth. [Full example](/examples/server/agentic-tool-rounds/)
...
If no `session_id` is passed, the server resolves or creates one and returns it in the response; see [sessions](/guides/agents/persist-sessions/) for matching rules, export/import, and lifetimes.

## Where to go next

- Gate actions behind user approval: [permissions and approvals](/guides/agents/permissions-and-approvals/).
- The same runtime in-process: [Python agentic tools example](/examples/python/agentic-tools/) and [Rust agent example](/examples/rust/advanced/agent/).
- Custom tools: run them as an [MCP (Model Context Protocol) server](/guides/agents/connect-mcp-server/) or register SDK callbacks ([tool calling](/guides/agents/tool-calling-basics/)).
