---
title: Skills
description: Upload and reference Skills from OpenAI-compatible Responses, Anthropic-compatible Messages, or local SDK mounts.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

inference.rs implements uploaded Skills for both the OpenAI-compatible Responses API and the Anthropic-compatible Messages API. You can upload a skill bundle to `/v1/skills`, reference it from a request, and let the model use the bundled instructions and files during the run.

This page documents the inference.rs compatibility surface. For the upstream API shapes and authoring model, see OpenAI's [Tools and Skills guide](https://developers.openai.com/api/docs/guides/tools-skills), Anthropic's [Agent Skills overview](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview), and Anthropic's [Skills API guide](https://platform.claude.com/docs/en/build-with-claude/skills-guide).
...
For executor flags, sandboxing, and approvals, see [shell execution](/guides/agents/enable-shell/) and [permissions and approvals](/guides/agents/permissions-and-approvals/). For request files used alongside Skills, see [OpenAI-compatible file inputs](/guides/agents/file-inputs/).
...
- inference.rs agentic extensions for Skills requests: `max_tool_rounds` to bound agentic tool loops, and `files` to require specific output files and surface them as downloadable File objects.
...
inference.rs implements the OpenAI-compatible server/runtime layer. It does not act as a separate skill registry client.
...
[Full server example](/examples/server/skills/)
...
<TabItem label="Anthropic server">
...
[Full server example](/examples/server/anthropic-skills/)
...
[Full example](/examples/python/shell-skills/)
...
[Full example](/examples/rust/advanced/shell-skills/)
...
The shell working directory is still controlled by `--shell-workdir`; without it, each session uses a per-session temp directory. Session state, tool records, and generated files follow the same lifecycle described in [persist sessions](/guides/agents/persist-sessions/).

## See also

- [OpenAI compatibility](/reference/openai-compatibility/): exact supported fields and known gaps.
- [Shell execution](/guides/agents/enable-shell/): executor configuration and sandboxing.
- [Build an agent](/guides/agents/build-an-agent/): combine search, code execution, shell, files, Skills, and sessions.
- Examples: [server upload](/examples/server/skills/), [Python local mount](/examples/python/shell-skills/), [Rust local mount](/examples/rust/advanced/shell-skills/).
