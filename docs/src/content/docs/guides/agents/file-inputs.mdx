...
inference.rs supports OpenAI-compatible user file inputs on the server API and local SDKs:

- [`POST /v1/files`](/reference/http-api-generated/operations/upload_file/) multipart uploads with `purpose="user_data"`.
- [`POST /v1/responses`](/reference/http-api-generated/operations/create_response/) `input_file` content parts with `file_id`, `file_data`, or `file_url`.
- [`POST /v1/chat/completions`](/reference/http-api-generated/operations/chatcompletions/) `file` content parts with `file_id` or `file_data`.
- Rust SDK [`InputFile`](/reference/rust-sdk/) on `RequestBuilder::with_input_file(...)`.
- Python SDK [`InputFile`](/reference/python/files/) on `ChatCompletionRequest.input_files`.
...
For field-level compatibility notes, see [OpenAI compatibility](/reference/openai-compatibility/). For wire schema and lifetime rules, see [HTTP API file semantics](/reference/http-api/#file-wire-schemas-and-semantics).
...
- [`GET /v1/files`](/reference/http-api-generated/operations/list_files/)
- [`GET /v1/files/{id}`](/reference/http-api-generated/operations/get_file/)
- [`GET /v1/files/{id}/content`](/reference/http-api-generated/operations/get_file_content/)
...
| Binary or structured documents | PDFs, images, archives, spreadsheets, and other binary formats are stored and mounted, but inference.rs does not extract OCR, PDF text, or spreadsheet summaries yet. |
...
- [Build an agent](/guides/agents/build-an-agent/) for the `--agent` preset.
- [Code execution](/guides/agents/enable-code-execution/) and [shell execution](/guides/agents/enable-shell/) for workdir mounting behavior.
- [OpenAI-compatible Skills](/guides/agents/skills/) for using request files alongside uploaded Skills.
- [Agentic runtime](/guides/agents/agentic-runtime/) for streaming progress, sessions, and file events.
...
- [Server API](/examples/server/file-inputs/)
- [Python SDK](/examples/python/file-inputs/)
- [Rust SDK](/examples/rust/advanced/file-inputs/)
