...
inference.rs detects text, multimodal, embedding, speech, and diffusion pipelines from supported
...
```rust
use anyhow::Result;
use inference::{ModelBuilder, TextMessageRole, TextMessages};
...
`inference-community/<model-name>-UQFF` when available and otherwise apply ISQ.
...
`inference tune`) and picks a level, or runs at full precision if the model fits. Choose an explicit
...
```bash
inference run -m /path/to/model-dir
```
...
HF_HUB_OFFLINE=1 inference serve -m Qwen/Qwen3-4B
```

Pre-download with `huggingface-cli download <repo>` or by running inference.rs once online;
`inference cache list` shows what is cached. Files resolve from `$HF_HUB_CACHE`, falling back to
