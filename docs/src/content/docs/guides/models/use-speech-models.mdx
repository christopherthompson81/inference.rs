...
inference.rs supports two speech-related model families:
...
```bash
inference serve -m mistralai/Voxtral-Mini-4B-Realtime-2602
```
...
```bash
inference serve -m nari-labs/Dia-1.6B
```
...
from inference_rs import Runner, SpeechLoaderType, Which
