...
Currently supported:
- **DiffusionGemma** (`google/diffusiongemma-26B-A4B-it`), a 26B-A4B [MoE (Mixture of Experts)](/guides/models/model-family-notes/) model with vision input, built on the Gemma 4 architecture.
...
```bash
inference run -m google/diffusiongemma-26B-A4B-it
```
...
```bash
inference serve -p 1234 -m google/diffusiongemma-26B-A4B-it
```
...
The standard chat-completion API works unchanged. See the [Python example](/examples/python/diffusion-gemma/) (vision input).
...
The standard chat API works unchanged. See the [Rust example](/examples/rust/models/diffusion-gemma/) (streaming, shows block-at-a-time output).

```rust
use inference::{MultimodalModelBuilder, TextMessageRole, TextMessages};
...
See also: [model family notes](/guides/models/model-family-notes/) and the [supported models reference](/reference/supported-models/).
