...
description: How inference.rs resolves a model's chat template, and how to override it when that goes wrong.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

A chat template formats messages into the string the model receives. An incorrect template can
degrade output or expose special tokens. inference.rs normally resolves the template from the model
files; when it cannot, provide a template file explicitly:

```bash
inference run -m <model> --chat-template my-template.jinja
```

## How the template is resolved

Template precedence, from highest to lowest, is `--jinja-explicit`, a multimodal
`processor_config.json` template, `--chat-template`, the model repository's template, and GGUF
metadata.
...
- For multimodal models, a `chat_template` in `processor_config.json` takes precedence unless
  `--jinja-explicit` is set.
...
```bash
inference run -m <model> --chat-template chat_templates/llama3.json
```
...
- `enable_thinking`: resolved thinking toggle; defaults to true when the request specifies neither control.
- `reasoning_effort`, `reasoning_strength`: the same explicit `off`, `low`, `medium`, `high`, or `xhigh` value. Both are undefined when effort is omitted.
...
Multimodal models need templates that handle non-text content parts, usually via placeholder tokens like `<|image|>` or `<|audio|>`; most multimodal repos ship theirs in `processor_config.json` or `chat_template.json`, which inference.rs picks up automatically.
