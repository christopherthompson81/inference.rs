...
For GGUF file and projector selection, see [Run GGUF models](/guides/models/run-gguf/). The [GGUF support reference](/reference/gguf-support/) summarizes adapter and multimodal boundaries.
...
The alias, request-routing, and lifecycle APIs below also apply to GGUF. Model files and companion
projectors follow the [GGUF selection rules](/guides/models/run-gguf/).
...
```bash
inference serve -m <base-model> --enable-lora
```
...
```python
from inference_rs import ChatCompletionRequest, LoraAdapter, Runner, Which
...
```rust
use inference::{LoraModelBuilder, RequestBuilder, TextMessageRole, TextModelBuilder};
...
Python and Rust require an exact GGUF filename. The
[GGUF guide](/guides/models/run-gguf/) covers automatic CLI selection.

<Tabs>
<TabItem label="Python">

```python
from inference_rs import LoraAdapter, Runner, Which
...
```rust
use inference::GgufModelBuilder;

let model = GgufModelBuilder::new("<gguf-repo>", vec!["<model.gguf>"])
    .with_lora_adapter("code", "<code-lora-repo>")
    .build()
    .await?;
```
...
These builders also support multimodal GGUF. Configure the model and companion assets as described
in the GGUF guide; adapters apply only to the language-model component.
...
Routed-expert adapters use the aliases, per-request selection, preload limits, and lifecycle API
described above.

```bash
inference serve -m Qwen/Qwen3-30B-A3B \
...
```bash
inference serve -m Qwen/Qwen3.6-35B-A3B \
  --lora domain=jeeejeee/qwen36-35ba3b-moe-all-linear-poken-lora
```

Use the default `auto` model selection for safetensors multimodal LoRA. Image options such as
...
The Rust and Python SDKs can load an adapter directory while a single-process model is serving requests. The directory must contain `adapter_config.json` and one `adapter_model.safetensors` file. The adapter must target the loaded base model; inference.rs checks compatibility but cannot prove that an adapter was trained from the correct base weights.
...
```rust
use inference::LoraAdapterLoadPolicy;
...
The read-only list route does not require mutation to be enabled. Runtime load and unload are available only when `INFERENCE_RS_ALLOW_RUNTIME_LORA_UPDATING` is enabled. `INFERENCE_RS_LORA_ADAPTER_ROOT` restricts `lora_path` to a canonical directory tree:

```bash
INFERENCE_RS_ALLOW_RUNTIME_LORA_UPDATING=1 \
INFERENCE_RS_LORA_ADAPTER_ROOT=/srv/adapters \
inference serve -m <base-model> --enable-lora
```

:::caution[Runtime loading reads local files]
The load endpoint reads paths from the server filesystem and consumes server memory. Set `INFERENCE_RS_LORA_ADAPTER_ROOT` in production and place authentication or an authenticated reverse proxy in front of load and unload. Relative `lora_path` values resolve beneath that root; without a root they resolve from the server working directory. The root, its ancestors, and its adapter directories must be operator-owned and not writable by API clients or other untrusted local users. Publish each complete adapter atomically at a new, content-versioned directory path, then leave that directory and its files unchanged while it may be loaded. Do not replace a selected directory path or modify its config or weights in place. Without an adapter root, an authorized client can request any readable adapter directory. Symlinks that resolve outside the configured root are rejected.
:::
...
| GGUF and GGML | GGUF supports dynamic LoRA for compatible language-model adapters. GGML uses legacy mode. See [GGUF support](/reference/gguf-support/) for legacy and multimodal boundaries. |
...
```bash
inference bench -m <base-model> --lora code=<adapter-repo> \
  --prompt-len 128,512 --gen-len 128 --iterations 5
inference bench -m <base-model> --lora code=<adapter-repo> --adapter code \
  --prompt-len 128,512 --gen-len 128 --iterations 5
```
...
Full examples: [lora](/examples/python/lora/) (Python), [lora](/examples/rust/advanced/lora/) (Rust), and [adapter-chat](/examples/server/adapter-chat/) (HTTP).

## Migrating from inference.rs 0.9.0 and older

Dynamic LoRA replaces the ordinary-LoRA API from inference.rs 0.9.0 and older. The previous path loaded one or more adapters as one static stack; the current path assigns every adapter an alias and selects zero or one adapter for each request.

| inference.rs 0.9.0 and older | Current API |
|---|---|
...
| Raw GGUF/GGML `--lora <source>` | For GGUF, use `--lora alias=source` and select the alias dynamically. For GGML or a compatible static GGUF configuration, use `--legacy-lora <source> --legacy-lora-order <ordering-file.json>`; the [ordering JSON](#legacy-raw-lora) is required. |
| Plain/safetensors TOML `lora = "repo"` | Use the structured `adapter.lora` list shown in the [TOML reference](/reference/cli-toml-config/). |
...
| vLLM | inference.rs |
|---|---|
| `vllm serve BASE --enable-lora --lora-modules code=SOURCE` | `inference serve -m BASE --lora-modules code=SOURCE`; `--lora` is the canonical spelling |
| `--enable-lora` with no preload | `--enable-lora` |
| `--max-lora-rank N` | Same spelling is accepted; `--lora-max-rank` is canonical |
| `VLLM_ALLOW_RUNTIME_LORA_UPDATING=True` | `INFERENCE_RS_ALLOW_RUNTIME_LORA_UPDATING=1` |
...
| `lora_runtime_unavailable` | Target a model with a dynamic LoRA runtime. For `inference serve`, use a preload or `--enable-lora`; also check tensor-parallel restrictions. |
...
| `adapter_path_forbidden` or `adapter_file_forbidden` | Move the immutable directory below `INFERENCE_RS_LORA_ADAPTER_ROOT` and fix its permissions. |
...
GGUF supports dynamic LoRA for compatible language-model adapters. Legacy mode remains available
for GGML and the static GGUF configurations listed in the
[GGUF support reference](/reference/gguf-support/).
...
```python
from inference_rs import Runner, Which
...
```rust
use std::fs::File;
use inference::{TextModelBuilder, XLoraModelBuilder};
...
Full examples: [xlora-zephyr](/examples/python/xlora-zephyr/) (Python), [xlora](/examples/rust/advanced/xlora/) (Rust).
...
- See the [AnyMoE Python reference](/reference/python/anymoe/) for configuration fields.

Full examples: [anymoe](/examples/python/anymoe/) (Python), [anymoe](/examples/rust/advanced/anymoe/) and [anymoe-lora](/examples/rust/advanced/anymoe-lora/) (Rust).
