...
Fields a request leaves unset fall back to the model's `generation_config.json` (temperature, top-k, top-p, min-p, repetition penalty). Without such defaults the engine samples at temperature 1.0 with no filters. Send `temperature: 0` (or `top_k: 1`) for greedy decoding.
...
Generation stops at the model's EOS tokens, a request-supplied `stop` string or token, or the token
limit (`max_tokens`), whichever comes first.

Set `ignore_eos: true` to continue after the model emits EOS. Explicit stops and the token limit
still apply, so set `max_tokens` or `stop` to bound the response. This option can be used with base
(non-instruct) models that emit EOS early or when measuring decode throughput.
...
Interactive mode (`inference run`) exposes slash commands that persist for the rest of the session:
...
Until overridden, interactive mode seeds its sampling from the model's `generation_config.json` when present, else temperature 0.8, top-k 40, top-p 0.95, min-p 0.05.
...
The engine-wide random seed is set with `--seed` on the CLI or `seed=` on the Python `Runner`. OpenAI-compatible generation requests can instead provide `seed` for a deterministic request-scoped stream. Each choice in an `n > 1` request receives a distinct deterministic stream.
