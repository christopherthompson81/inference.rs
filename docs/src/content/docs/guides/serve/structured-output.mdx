...
inference.rs constrains generation server-side with [llguidance](https://github.com/guidance-ai/llguidance), so the output is guaranteed to match your schema -- no retry loops or post-hoc validation. The common case is a JSON schema; regex, Lark, and raw llguidance grammars cover everything else.
...
Full examples: [openai_response_format](/examples/server/openai-response-format/), [json_schema](/examples/server/json-schema/).
...
Full examples: [pydantic_schema](/examples/python/pydantic-schema/), [json_schema](/examples/python/json-schema/).
...
Full examples: [cookbook/structured](/examples/rust/cookbook/structured/), [advanced/json_schema](/examples/rust/advanced/json-schema/).

</TabItem>
</Tabs>

For JSON-Schema-constrained *tool arguments* (rather than the whole response), set `strict: true` on the function tool; see [tool calling](/guides/agents/tool-calling-basics/).
...
Full examples: [regex](/examples/server/regex/), [lark](/examples/server/lark/), [llguidance](/examples/server/llguidance/) (HTTP); [regex](/examples/python/regex/), [lark_llg](/examples/python/lark-llg/) (Python); [grammar](/examples/rust/advanced/grammar/), [llguidance](/examples/rust/advanced/llguidance/) (Rust).

## Notes

- `response_format` accepts `{"type": "text"}` (no constraint) and `{"type": "json_schema", ...}`. OpenAI's `json_object` mode is not accepted; supply a schema instead. See [OpenAI compatibility](/reference/openai-compatibility/) for field-level deviations.
