...
description: Configure coding agents to use a local inference.rs server.
---

inference.rs can back coding-agent clients through the compatibility APIs those
...
```bash
inference serve -p 1234 -m Qwen/Qwen3-Coder-Next
```
...
commands, and repository inspection. Enable inference.rs server-side agent tools
only when you deliberately want web search or Python code execution to happen
inside the inference.rs server as well.
...
To keep inference.rs as a switchable alternative instead, move the `model` and
...
```json
{
  ...
  "env": {
    ...
    "ANTHROPIC_CUSTOM_MODEL_OPTION_NAME": "inference.rs default",
    "ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION": "Local model served by inference.rs",
...
inference.rs model. If you serve several models, map each Claude Code default to
...
`tool_result` content blocks. inference.rs translates these to its internal tool
...
inference.rs tools are separate:

| Feature | How it reaches inference.rs |
|---|---|
...
Server-side web search and code execution require the corresponding inference.rs
...
| A remote server accepts any key | Put authentication in a reverse proxy. inference.rs does not validate compatibility API keys. |
