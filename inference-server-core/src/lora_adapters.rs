...
impl LoraAdapterApiError {
    ...
    fn core(error: InferenceRsError) -> Self {
        match error {
            InferenceRsError::LoraAdapter(error) => Self::lora(error),
            error @ InferenceRsError::ModelNotFound(_) => {
                Self::new(StatusCode::NOT_FOUND, "model_not_found", error.to_string())
            }
            error @ InferenceRsError::ModelReloading(_)
            | error @ InferenceRsError::ModelAlreadyLoaded(_)
            | error @ InferenceRsError::ModelAlreadyUnloaded(_) => Self::new(
                StatusCode::CONFLICT,
                "model_state_conflict",
                error.to_string(),
            ),
            error @ InferenceRsError::NoLoaderConfig(_) => Self::new(
                StatusCode::BAD_REQUEST,
                "invalid_model_operation",
                error.to_string(),
            ),
            error @ InferenceRsError::SenderPoisoned => Self::new(
                StatusCode::SERVICE_UNAVAILABLE,
                "service_unavailable",
                error.to_string(),
            ),
            error @ InferenceRsError::EnginePoisoned
            | error @ InferenceRsError::ReloadFailed(_)
            | error @ InferenceRsError::Other(_) => Self::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                "internal_error",
                error.to_string(),
            ),
...
mod tests {
    ...
    fn core_errors_map_to_stable_http_statuses() {
        assert_eq!(
            LoraAdapterApiError::core(InferenceRsError::ModelNotFound("model".to_string())).status,
