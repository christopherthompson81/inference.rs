...
pub struct AnthropicMessagesRequest {
    ...
    /// inference.rs extension for toggling reasoning.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub enable_thinking: Option<bool>,
    /// inference.rs extension for local reasoning effort. "none" aliases "off"; "max" aliases "xhigh".
...
pub struct AnthropicMessageResponse {
    ...
    pub files: Option<Vec<inference_core::File>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub session_id: Option<String>,
}
...
fn resolve_anthropic_response_format(
    ...
    if native.is_some() && extension.is_some() {
        anyhow::bail!(
            "Anthropic `output_config.format` conflicts with inference.rs `response_format`."
        );
    }
...
impl AnthropicMessagesRequest {
    ...
    fn into_chat_completion_request(self) -> Result<ChatCompletionRequest> {
        ...
        inference_core::resolve_reasoning_controls(enable_thinking, reasoning_effort)?;
        let response_format =
    ...
    fn try_from(skill: AnthropicSkillReference) -> Result<Self> {
        match skill.source.as_str() {
            ...
            "anthropic" => anyhow::bail!(
                "Anthropic-managed skills are not bundled with inference.rs; upload a custom skill and use `type`: `custom`."
            ),
...
impl AnthropicStreamState {
    ...
    fn enqueue_tool_use(&mut self, tool_call: &inference_core::ToolCallResponse) {
        self.stop_thinking_block();
        self.stop_text_block();
        let index = self.next_content_index;
        self.next_content_index += 1;
                    ...
                    match resp {
                        ...
                        Response::ValidationError(e) => {
                            ...
                            if matches!(
                                ...
                                InferenceRs::maybe_log_error(self.state.clone(), e.as_ref());
                            }
...
mod tests {
    ...
    use inference_core::{
        CalledFunction, Choice, ChunkChoice, Delta, InferenceRsError, PromptTokensDetails,
    ...
    #[test]
    fn routes_inference_sampling_and_constraint_extensions() {
        let req: AnthropicMessagesRequest = serde_json::from_value(json!({
            "model": "default",
            "max_tokens": 32,
            "messages": [{"role": "user", "content": "Return JSON."}],
