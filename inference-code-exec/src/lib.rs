...
use inference_sandbox::{NetworkMode, Sandbox, SandboxPolicy};
...
pub struct CodeExecutionConfig {
    ...
    #[serde(default)]
    pub sandbox_policy: Option<inference_sandbox::SandboxPolicy>,
    #[serde(default)]
...
async fn sandbox_for_config(
    ...
    let (sandbox, mut policy): (Arc<dyn Sandbox>, SandboxPolicy) =
        match config.sandbox_policy.clone() {
            Some(policy) => (Arc::from(inference_sandbox::detect()), policy),
            None => (
                Arc::from(inference_sandbox::null()),
...
impl CodeExecutionManager {
    ...
    pub fn network_mode(&self) -> Option<inference_sandbox::NetworkMode> {
    ...
    pub fn effective_protection(&self) -> inference_sandbox::EffectiveProtection {
    ...
    pub fn get_tool_callbacks(&self, input_modalities: &[InputModality]) -> ToolCallbacksWithTools {
        ...
        let execute_callback: Arc<inference_mcp::MultimodalToolCallback> = Arc::new(
            move |func: &CalledFunction, tc: &inference_mcp::ToolCallContext| {
        ...
        let reset_callback: Arc<inference_mcp::ToolCallback> = Arc::new(
            move |_func: &CalledFunction, tc: &inference_mcp::ToolCallContext| {
                let sessions = Arc::clone(&sessions);
                let ctx = ctx.clone();
        ...
        let read_file_stub: Arc<inference_mcp::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
        ...
        let list_files_stub: Arc<inference_mcp::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
...
fn denied_by_permission(
    ctx: &SpawnCtx,
    tool_ctx: &inference_mcp::ToolCallContext,
    session_id: &str,
    code: &str,
    outputs: &[String],
) -> Option<String> {
    ...
    let reason = match permission {
        ...
        CodeExecutionPermission::Ask => {
            ...
            if let Some(notifier) = &tool_ctx.code_execution_approval_notifier {
                notifier(inference_mcp::CodeExecutionApprovalRequest {
                    approval_id,
...
mod tests {
    ...
    impl Sandbox for FakeSandbox {
        ...
        ) -> Result<(), inference_sandbox::SandboxError> {
            Ok(())
        }
        ...
        fn effective(&self, _policy: &SandboxPolicy) -> inference_sandbox::EffectiveProtection {
            inference_sandbox::EffectiveProtection {
