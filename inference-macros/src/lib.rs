//! Proc macros for ergonomic tool definition in inference.rs
//!
//! This crate provides the `#[tool]` attribute macro for defining tools
//! that can be used with the inference.rs agentic loop.
//!
//! # Example
//!
//! ```ignore
//! use inference_macros::tool;
...
/// used with the inference.rs agentic loop. It generates:
///
...
fn generate_tool_impl(args: ToolArgs, input_fn: ItemFn) -> syn::Result<TokenStream2> {
    ...
    let output = if is_async {
        ...
        quote! {
            ...
            #fn_vis fn #tool_fn_name() -> inference::Tool {
                ...
                inference::Tool {
                    tp: inference::ToolType::Function,
