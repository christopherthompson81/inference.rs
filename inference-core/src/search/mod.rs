...
pub(crate) const SEARCH_TOOL_NAME: &str = "inference_search_the_web";
pub(crate) const EXTRACT_TOOL_NAME: &str = "inference_website_content_extractor";
...
pub async fn run_search_tool(params: &SearchFunctionParameters) -> Result<Vec<SearchResult>> {
    ...
    let user_agent = format!("inference/{APP_VERSION} ({OS}; {ARCH}; {FAMILY})");
