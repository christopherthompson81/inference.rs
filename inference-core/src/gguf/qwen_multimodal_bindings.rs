...
mod tests {
    ...
    fn qwen35_multimodal_config_selects_tiled_gdn() -> Result<()> {
        for loader_type in [
            ...
            let config = normalize_qwen_multimodal_config(
                &loader_type,
                r#"{"text_config":{"_inference_gdn_v_head_layout":"grouped"}}"#,
            )?;
    ...
    fn local_qwen35moe_archives_have_complete_native_bindings() -> Result<()> {
        let path = std::env::var("INFERENCE_RS_QWEN35_GGUF")
            .map_err(|_| anyhow::anyhow!("INFERENCE_RS_QWEN35_GGUF is not set"))?;
        let mmproj_path = std::env::var("INFERENCE_RS_QWEN35_MMPROJ_GGUF")
            .map_err(|_| anyhow::anyhow!("INFERENCE_RS_QWEN35_MMPROJ_GGUF is not set"))?;
