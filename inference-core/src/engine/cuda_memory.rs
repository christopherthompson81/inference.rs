...
pub(super) fn record_prompt_batch_reduction(previous: usize, retained: usize) {
    metrics::counter!("inference_cuda_prompt_batch_reductions_total").increment(1);
    metrics::counter!("inference_cuda_prompt_sequences_deferred_total")
        ...
        let outcomes = self.devices.iter_mut().map(|maintained| {
            match maintain_device(maintained, point, transient_bytes) {
                ...
                Err(err) => {
                    ...
                    metrics::counter!(
                        "inference_cuda_memory_maintenance_total",
                        "device" => device.clone(),
                        "reason" => point.label(),
                        "action" => "maintain",
                        "outcome" => "error"
...
fn maintain_device(
    ...
    metrics::gauge!(
        "inference_cuda_memory_required_available_bytes",
...
fn record_allocator_metrics(device: &Device, snapshot: CudaAllocatorSnapshot) {
    ...
    for (state, value) in [("total", snapshot.total), ("available", snapshot.available)] {
        metrics::gauge!(
            "inference_cuda_device_memory_bytes",
            "device" => device.clone(),
            "state" => state
        )
        .set(value as f64);
    ...
    if let Some(pool) = snapshot.async_pool {
        for (state, watermark, value) in [
            ...
            metrics::gauge!(
                "inference_cuda_memory_pool_bytes",
                "device" => device.clone(),
                "pool" => "async",
                "state" => state,
                "watermark" => watermark
        ...
        metrics::gauge!(
            "inference_cuda_memory_pool_release_threshold_bytes",
            "device" => device.clone(),
            "pool" => "async"
        )
...
fn record_reclaimed(
    ...
    let outcome = if reclaimed == 0 {
        ...
        metrics::counter!(
            "inference_cuda_memory_reclaimed_bytes_total",
...
fn record_pressure(device: &Device, level: &'static str) {
    metrics::counter!(
        "inference_cuda_memory_pressure_total",
        "device" => device.device_pretty_repr(),
        "level" => level
    )
    .increment(1);
...
fn record_pending(device: &Device, pending: bool) {
    metrics::gauge!(
        "inference_cuda_memory_maintenance_pending",
        "device" => device.device_pretty_repr()
    )
