...
impl PagedAttentionScheduler {
    ...
    fn finish_front_of_waiting(
        ...
        metrics::counter!("inference_sequences_rejected_total", "reason" => metric_reason)
            .increment(1);
    ...
    pub fn schedule(
        ...
        while !completion_due && !self.waiting.is_empty() {
            ...
            let clamped = {
                ...
                clamp_prefix_cache_hit_len(clamped, self.block_size, seq_guard.mm_features())
            };
            ...
            metrics::counter!("inference_prefix_cache_tokens_matched_total").increment(
                u64::try_from(matched_prefix_tokens).expect("prefix length exceeds u64"),
            );
            metrics::counter!("inference_prefix_cache_tokens_reused_total")
    ...
    fn publish_kv_block_metrics(&self) {
        ...
        metrics::gauge!("inference_kv_cache_blocks_used").set(active as f64);
        metrics::gauge!("inference_kv_cache_blocks_total").set(total as f64);
        metrics::gauge!("inference_kv_cache_blocks_active").set(active as f64);
        metrics::gauge!("inference_kv_cache_blocks_reserved")
            .set(kv_mgr.num_reserved_blocks() as f64);
        metrics::gauge!("inference_kv_cache_blocks_available")
            .set(kv_mgr.num_unreserved_blocks() as f64);
        metrics::gauge!("inference_kv_cache_blocks_prefix_cached")
            .set(kv_mgr.num_prefix_cached_blocks() as f64);
        metrics::gauge!("inference_kv_cache_blocks_prefix_retained")
            .set(kv_mgr.num_retained_prefix_blocks() as f64);
    }

    pub fn free_finished_sequence_groups(&mut self) {
        let mut finished = Vec::new();
        let mut cacheable_finished = Vec::new();
        for seq in self.running.iter().chain(self.waiting.iter()) {
            ...
            if seq_guard.is_finished_paged_attn() {
                ...
                let num_computed_tokens = seq_guard.num_computed_tokens();
                if self.prefix_caching_enabled
                    && !matches!(seq_guard.getstate(), SequenceState::Error)
                    && num_computed_tokens >= self.block_size
                {
                    cacheable_finished.push((seq.clone(), id, num_computed_tokens));
                }
                finished.push(id);
            }
        }
        ...
        if self.prefix_caching_enabled {
            for (seq, id, num_computed_tokens) in cacheable_finished {
                let seq_guard = get_mut_arcmutex!(seq);
                self.ensure_block_hashes(
                    id,
                    seq_guard.get_toks(),
...
impl PagedAttentionScheduler {
    fn _preempt(&mut self, seq: Arc<Mutex<Sequence>>) {
        ...
        metrics::counter!("inference_paged_preemptions_total").increment(1);
        if seq_guard.active_staged_speculative_len() > 0 {
            metrics::counter!("inference_speculative_staged_drops_total").increment(1);
        }
