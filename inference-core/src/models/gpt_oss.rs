...
fn gptoss_swiglu(gate: &Tensor, up: &Tensor, alpha: f32, limit: f32) -> Result<Tensor> {
    ...
    if gate.device().is_cuda() {
        return inference_quant::gptoss_swiglu_fused(gate, up, alpha, limit);
    }
...
struct GptOssMoE {
    ...
    expert_lora: Option<Arc<inference_quant::LoraExpertSiteHandle>>,
...
impl GptOssMoE {
    fn new(cfg: &Config, vb: ShardedVarBuilder, layer_device: Device) -> Result<Self> {
        ...
        let gate_lora = inference_quant::register_dynamic_lora_site(
            &gate_vb,
            inference_quant::LoraLinearSpec::replicated(cfg.hidden_size, cfg.num_local_experts),
        )?;
        ...
        let expert_lora = match experts_vb.lora_registry() {
            Some(registry) => Some(
                registry.register_expert(
                    inference_quant::LoraSiteKey::new(experts_vb.prefix()),
                    inference_quant::LoraExpertSiteSpec::new(
                        cfg.num_local_experts,
                        cfg.hidden_size,
                        cfg.intermediate_size,
                        inference_quant::LoraExpertProjectionNames::new(
                            "gate_proj",
                            "up_proj",
                            "down_proj",
                        ),
                        inference_quant::Shard::default(),
                        inference_quant::Shard::default(),
                    )?
                    .with_gate_up_order(inference_quant::LoraGateUpOrder::Interleaved),
                    experts_vb.dtype(),
    ...
    fn forward(&self, xs: &Tensor, _layer_idx: usize) -> Result<Tensor> {
        ...
        let expert_lora = self
            .expert_lora
            .as_ref()
            .map(inference_quant::LoraExpertExecution::current)
            .transpose()?
            .flatten();
            ...
            GptOssExpertProjections::Split { gate, up, .. } => {
                ...
                if let Some(lora) = &expert_lora {
                    gate = lora.add_delta_owned(
                        inference_quant::LoraExpertProjection::Gate,
                        &xs_flat,
                        gate,
                        &topk_ids,
                        None,
                        inference_quant::LoraExpertInputMode::TokenRows,
                    )?;
                    up = lora.add_delta_owned(
                        inference_quant::LoraExpertProjection::Up,
                        &xs_flat,
                        up,
                        &topk_ids,
                        None,
        ...
        let expert_out = match &expert_lora {
            Some(lora) => lora.add_delta_owned(
                inference_quant::LoraExpertProjection::Down,
                &activated,
                expert_out,
                &topk_ids,
                None,
                inference_quant::LoraExpertInputMode::RoutedRows,
            )?,
            None => expert_out,
        };
...
mod tests {
    ...
    fn residual_test_model() -> Result<Model> {
        ...
        let vb = inference_quant::ShardedSafeTensors::wrap(tensors, DType::F32, Device::Cpu);
