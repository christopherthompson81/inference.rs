...
const UQFF_METADATA_INFERENCE_RS_VERSION: &str = "uqff.producer.inference.version";
const UQFF_METADATA_INFERENCE_RS_GIT_REVISION: &str = "uqff.producer.inference.git_revision";
const UQFF_STAGING_PREFIX: &str = ".inference-rs-uqff-";
...
fn existing_report_artifacts(parent: &Path, stems: &[String]) -> (bool, Vec<PathBuf>) {
    let report_path = parent.join(inference_quant::UQFF_REPORT_JSON);
    let Ok(contents) = std::fs::read(report_path) else {
        return (false, Vec::new());
    };
    let Ok(report) = serde_json::from_slice::<inference_quant::UqffReport>(&contents) else {
        return (false, Vec::new());
    };
    let shards = report
        .outputs
...
pub(crate) fn write_uqff_artifacts(request: UqffWriteRequest<'_>) -> Result<()> {
    ...
    let report = inference_quant::UqffReport {
        ...
        generated_by: inference_quant::UqffGeneratedBy {
            tool: "inference quantize".to_string(),
            inference_version: Some(crate::INFERENCE_RS_VERSION.to_string()),
            git_revision: Some(crate::INFERENCE_RS_GIT_REVISION.to_string()),
        },
        base_model,
        repo_id,
        uqff_version: format!(
            "{}.{}.{}",
            inference_quant::UQFF_VERSION_MAJOR,
            inference_quant::UQFF_VERSION_MINOR,
            inference_quant::UQFF_VERSION_PATCH
        ),
        outputs: report_outputs,
    };
    inference_quant::write_uqff_report(&staging.payload, &report)?;
    staging.publish()?;
...
fn write_uqff_type(ctx: UqffTypeWriteContext<'_>) -> Result<inference_quant::UqffOutputReport> {
    ...
    let mut shards = Vec::new();
    let quant_report = inference_quant::QuantizationReport::default();
    let mut layer_reports = Vec::with_capacity(layers.len());

    for version in inference_quant::uqff_version_tensors() {
    ...
    let handles = inference_quant::requantize_tracked(
        ...
        inference_quant::IsqConsumer::UqffWrite,
        MAX_UQFF_SIZE_BYTES,
    ...
    for module in layers {
        ...
        let (stored, shape) =
            inference_quant::stored_type_from_tensors(&serialized_tensors, &module.key)?;
        layer_reports.push(inference_quant::UqffLayerReport {
    ...
    let output_report = inference_quant::build_output_report_from_layers(
        ty.to_string(),
        shards,
        layer_reports,
        &quant_report,
...
fn uqff_safetensors_metadata() -> HashMap<String, String> {
    HashMap::from([
        ...
        (
            UQFF_METADATA_INFERENCE_RS_VERSION.to_string(),
            crate::INFERENCE_RS_VERSION.to_string(),
        ),
        (
            UQFF_METADATA_INFERENCE_RS_GIT_REVISION.to_string(),
...
fn gguf_imatrix_names(key: &str) -> Vec<String> {
    ...
    let Some(index) = inference_quant::layer_index_from_prefix(key) else {
        return Vec::new();
    };
    let per_expert = key
        .split_once(".experts.")
...
mod tests {
    ...
    fn uqff_config_removes_source_quantization_metadata_recursively() -> Result<()> {
        let source = serde_json::json!({
            ...
            "_inference_qk_rope_layout": "adjacent",
        ...
        assert_eq!(sanitized["_inference_gdn_v_head_layout"], "tiled");
        assert_eq!(sanitized["_inference_qk_rope_layout"], "adjacent");
    ...
    fn write_test_uqff_report(parent: &Path, shards: &[&str]) -> Result<()> {
        ...
        std::fs::write(
            parent.join(inference_quant::UQFF_REPORT_JSON),
    ...
    fn uqff_publish_replaces_complete_shard_set_and_preserves_unrelated_files() -> Result<()> {
        ...
        let unrelated_report = std::fs::read(final_parent.join(inference_quant::UQFF_REPORT_JSON))?;
        ...
        assert_eq!(
            std::fs::read(final_parent.join(inference_quant::UQFF_REPORT_JSON))?,
            unrelated_report
        );
