...
pub(crate) struct IsqPlanInputs<'a> {
    ...
    pub topology_overrides: Vec<inference_quant::ImmediateIsqOverride>,
...
pub(crate) struct IsqLoadPlan {
    ...
    pub capture: inference_quant::IsqCaptureMode,
...
impl IsqLoadPlan {
    pub fn validate_tracked_selection(
        &self,
        modules: &[inference_quant::TrackedModule],
    ) -> Result<()> {
...
pub(crate) fn resolve_and_install_isq_plan(i: IsqPlanInputs<'_>) -> Result<IsqLoadPlan> {
        ...
        i.topology_overrides
            ...
            .filter_map(|override_entry| {
                override_entry
                    .device
                    .as_ref()
                    .map(|device| inference_quant::ImmediateIsqOverride {
    ...
    if allow_immediate_cli {
        ...
        let (executor, num_threads) = inference_quant::create_isq_executor(
            inference_quant::IsqExecutorConfig::new(immediate_ty),
        );
        tracing::debug!("Using {num_threads} worker thread(s) for weight quantization.");
        inference_quant::set_immediate_isq_config(
            inference_quant::ImmediateIsqConfig::new(immediate_ty, immediate_predicates, capture)
        ...
        tracing::debug!("Using {num_threads} worker thread(s) for weight quantization.");
        inference_quant::set_immediate_isq_config(
            inference_quant::ImmediateIsqConfig::new(
                immediate_ty,
                Vec::new(),
...
fn capture_mode(has_write_uqff: bool, wants_imatrix: bool) -> inference_quant::IsqCaptureMode {
    if has_write_uqff {
        inference_quant::IsqCaptureMode::CaptureAll
...
mod tests {
    ...
    fn tracked_module(key: &str, ty: Option<IsqType>) -> inference_quant::TrackedModule {
        let (_tx, rx) = inference_quant::pending_isq_channel();
        inference_quant::TrackedModule {
            key: key.to_string(),
            ct: std::sync::Arc::new(inference_quant::PendingIsqLayer::new(rx)),
            ty,
    ...
    fn typed_topology_override_allows_moqe_without_model_predicates() -> Result<()> {
        let plan = resolve_and_install_isq_plan(IsqPlanInputs {
            ...
            loading_from_uqff: false,
            organization: IsqOrganization::MoeExpertsOnly,
            topology_overrides: vec![inference_quant::ImmediateIsqOverride {
    ...
    fn uqff_precedence_suppresses_isq_topology_overrides() -> Result<()> {
        ...
        assert!(inference_quant::get_immediate_isq().is_none());
        Ok(())
    }
    ...
    fn uqff_precedence_preserves_topology_device_overrides() -> Result<()> {
        ...
        assert!(plan.immediate_isq_installed);
        assert!(!plan.loading_isq);
        let params = inference_quant::get_immediate_isq().expect("device override is installed");
    ...
    fn quantized_prepared_sources_keep_immediate_isq_enabled() -> Result<()> {
        ...
        assert!(plan.immediate_isq_installed);
        assert!(!plan.loading_isq);
        assert!(inference_quant::get_immediate_isq().is_some());
        inference_quant::clear_immediate_isq();
        Ok(())
    }
