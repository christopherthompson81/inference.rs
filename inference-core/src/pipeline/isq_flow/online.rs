...
pub(crate) fn apply_calibration(
    ...
    weight_source: Option<&dyn inference_quant::QuantizedWeightSource>,
    ...
    if let Some(path) = save_cimatrix {
        inference_quant::CollectedImatrixData(map.clone()).save_imatrix(path)?;
...
fn needs_distributed_wrapper(module: &TrackedModule) -> bool {
    let multi_rank = matches!(
        module.shard,
        Some(inference_quant::Shard::Simple { world_size, .. }) if world_size > 1
    );
    multi_rank
        && matches!(
            module.ct.is_distributed(),
            Some(inference_quant::DistributedKind::RowParallel)
        )
}

fn requantize_from_weight_source(
    modules: &[TrackedModule],
    source: &dyn inference_quant::QuantizedWeightSource,
    pool_ty: IsqType,
    ...
    let guard = inference_quant::QuantizeOntoGuard::new();
...
struct SourceWeights {
    mmap: std::sync::Arc<inference_quant::safetensors::MmapedSafetensors>,
...
impl SourceWeights {
    fn open(files: &[std::path::PathBuf]) -> Result<Self> {
        ...
        let mmap = std::sync::Arc::new(unsafe {
            inference_quant::safetensors::MmapedSafetensors::multi(files)?
        });
...
pub(crate) fn requantize_from_source(
    ...
    let (executor, _) = inference_quant::create_isq_executor(
        inference_quant::IsqExecutorConfig::new(Some(pool_ty)),
    );
    let guard = inference_quant::QuantizeOntoGuard::new();
    ...
    for module in dense {
        ...
        let request = inference_quant::IsqRequest {
            ty: Some(ty),
            device: device.clone(),
            has_imatrix: imatrix.is_some(),
            capture: inference_quant::IsqCaptureMode::Immediate,
            consumer: inference_quant::IsqConsumer::RuntimeSwap,
        ...
        let rx = executor.submit(plan, request.consumer, move || {
            let job = || -> Result<()> {
                ...
                let range = inference_quant::shard_range(shard, &source_shape)?;
                let b = if source_has_bias && (resident_has_bias || source_shape.len() == 3) {
                    let b = mmap.load(&format!("{}.bias", module.key), &Device::Cpu, None)?;
                    match inference_quant::bias_shard(range, source_shape.len()) {
                        inference_quant::BiasShard::Skip if source_shape.len() == 3 => {
                            anyhow::bail!(
                                "Biased stacked expert `{}` cannot be input-sharded; load replicated experts.",
                                module.key
                            );
                        }
                        inference_quant::BiasShard::Skip => None,
                        inference_quant::BiasShard::Full => Some(b),
                        inference_quant::BiasShard::Narrow { dim, start, len } => {
                ...
                let replacement = if w.rank() == 3 {
                    inference_quant::quantize_expert_stack_with_bias(
                        w,
                        b,
                        ty,
                        imatrix,
    ...
    for module in &experts {
        let job = || -> Result<()> {
            ...
            let range = inference_quant::shard_range(shard, projection.weight.dims())?;
            let weight_rank = projection.weight.rank();
            let weight = shard.apply_to(&projection.weight)?.force_contiguous()?;
            let bias = match (
                projection.bias,
                inference_quant::bias_shard(range, weight_rank),
            ) {
                (Some(_), inference_quant::BiasShard::Skip) => {
                    anyhow::bail!(
                        "Biased stacked expert `{}` cannot be input-sharded; load replicated experts.",
                        module.key
                    );
                }
                (None, inference_quant::BiasShard::Skip) => None,
                (bias, inference_quant::BiasShard::Full) => bias,
                (Some(bias), inference_quant::BiasShard::Narrow { dim, start, len }) => {
                    Some(bias.narrow(dim, start, len)?.contiguous()?)
                }
                (None, inference_quant::BiasShard::Narrow { .. }) => None,
            };
            let replacement = inference_quant::quantize_expert_stack_with_bias(
                weight,
                bias,
                ty,
                imatrix,
...
mod tests {
    ...
    impl inference_quant::QuantizedWeightSource for TestWeightSource {
    ...
    fn calibration_prefers_quantized_source_over_checkpoint_paths() -> Result<()> {
        ...
        let (tx, rx) = inference_quant::pending_isq_channel();
        tx.send(Ok(inference_quant::IsqJobOutput::ready(resident)))?;
        let ct = Arc::new(inference_quant::PendingIsqLayer::new(rx));
        ...
        let source: Arc<dyn inference_quant::QuantizedWeightSource> = Arc::new(TestWeightSource {
    ...
    fn from_source_respects_shard() -> Result<()> {
        ...
        tx.send(Ok(inference_quant::IsqJobOutput::ready(resident)))
            .unwrap();
        let ct = std::sync::Arc::new(inference_quant::PendingIsqLayer::new(rx));
        let module = TrackedModule {
            key: "m.lin".to_string(),
            ct: ct.clone(),
            ty: Some(IsqType::Q8_0),
    ...
    fn from_source_replaces_expert_layer() -> Result<()> {
        ...
        let module = TrackedModule {
            ...
            ty: Some(IsqType::Q8_0),
            promote_default: false,
            shard: Some(inference_quant::Shard::default()),
        };
    ...
    fn from_source_rejects_biased_expert_input_shard() -> Result<()> {
        ...
        let module = TrackedModule {
            ...
            ct: Arc::new(inference_quant::PendingIsqLayer::new(rx)),
            ty: Some(IsqType::Q8_0),
            promote_default: false,
            shard: Some(Shard::Simple {
                dim: 2,
