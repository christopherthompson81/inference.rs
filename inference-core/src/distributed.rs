...
pub(crate) const IS_DAEMON_FLAG: &str = "__INFERENCE_RS_DAEMON_INTERNAL";
const INFERENCE_RS_MN_GLOBAL_WORLD_SIZE: &str = "INFERENCE_RS_MN_GLOBAL_WORLD_SIZE";
const INFERENCE_RS_MN_LOCAL_WORLD_SIZE: &str = "INFERENCE_RS_MN_LOCAL_WORLD_SIZE";
...
fn requested_tensor_parallel_size(use_nccl: bool, use_ring: bool) -> anyhow::Result<Option<usize>> {
    ...
    if use_nccl {
        return Ok(Some(
            inference_quant::distributed::get_global_tp_size_from_devices()?,
        ));
    }
    Ok(None)
}
...
fn tensor_parallel_size_is_explicit(use_ring: bool) -> bool {
    use_ring
        || env::var(INFERENCE_RS_MN_GLOBAL_WORLD_SIZE).is_ok()
        || env::var(INFERENCE_RS_MN_LOCAL_WORLD_SIZE).is_ok()
}
...
fn validate_model_tensor_parallelism(
    ...
    for layer_idx in 0..model_config.num_layers() {
        ...
        inference_quant::validate_tp_head_layout(spec.q_heads, spec.kv_heads, world_size)
...
pub fn nccl_daemon_replicator_inference(inference: Arc<crate::InferenceRs>) {
    std::thread::spawn(move || {
        let rt = Runtime::new().unwrap();
        rt.block_on(async move {
            use interprocess::local_socket::traits::Stream;
...
pub fn ring_daemon_replicator_inference(inference: Arc<crate::InferenceRs>) {
    let ring_config = RingConfig::load();
...
pub fn begin_tensor_parallel_session(model_slots: usize) -> anyhow::Result<()> {
    if model_slots == 0 || is_daemon() || !inference_quant::distributed::use_nccl() {
        return Ok(());
    }
...
fn spawn_tensor_parallel_workers(ids: &[inference_quant::Id]) -> anyhow::Result<()> {
    let num_workers = inference_quant::distributed::get_global_tp_size_from_devices()? - 1;
    if num_workers == 0 {
        return Ok(());
    }
...
fn id_to_payload(id: &inference_quant::Id) -> BigCCharArray {
...
fn ensure_worker_session(ids: Option<&[BigCCharArray]>) {
    ...
    let ids = ids
        .iter()
        .map(|id| inference_quant::Id::uninit(id.0))
        .collect::<Vec<_>>();
    let _ = TP_SESSION.set(Mutex::new(TensorParallelSession {
        ids,
        next_model_index: 0,
...
fn next_tensor_parallel_id() -> anyhow::Result<Option<inference_quant::Id>> {
...
pub(crate) fn ipc_name() -> anyhow::Result<Name<'static>> {
    let printname = "inference_daemon.sock";
...
pub(crate) fn prepare_distributed_mapper<T: DeviceMappedModelLoader + IsqModelLoader + ?Sized>(
    ...
    let global_world_size = if let Some(world_size) = global_world_size_override {
        world_size
    } else if let Ok(x) = std::env::var(INFERENCE_RS_MN_GLOBAL_WORLD_SIZE) {
        usize::from_str(&x).context("INFERENCE_RS_MN_GLOBAL_WORLD_SIZE")?
    } else {
    ...
    let use_multi_node = std::env::var(INFERENCE_RS_MN_GLOBAL_WORLD_SIZE).is_ok();
    if use_multi_node {
        info!("INFERENCE_RS_MN_GLOBAL_WORLD_SIZE is set, entering multi-node.");
    }
    ...
    let local_rank = if let Ok(payload) = env::var(IS_DAEMON_FLAG) {
        ...
        id = next_tensor_parallel_id()?.unwrap_or_else(|| inference_quant::Id::uninit(new_id.0));

        send_worker_ready()?;
        worker_rank + 1
    } else if cfg!(feature = "ring") {
        id = inference_quant::Id::new();
    ...
    if use_multi_node {
        if let Ok(n_nodes) = env::var("INFERENCE_RS_MN_HEAD_NUM_WORKERS") {
        ...
        } else if let Ok(addr) = env::var("INFERENCE_RS_MN_WORKER_SERVER_ADDR") {
            info!("Worker node connecting to {addr}.");
            let client = inference_quant::Client::new(addr.parse()?, local_world_size)?;
    ...
    let comm = inference_quant::Comm::from_device(
        id,
        device,
