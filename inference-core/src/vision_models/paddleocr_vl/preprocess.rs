...
//! `image` crate's `FilterType::CatmullRom` (inference.rs's production resampler for `resample=3`),
...
/// `grid_thw = (t=1, gh, gw)`). The CatmullRom resize is inference.rs's `resample=3` path and is NOT
