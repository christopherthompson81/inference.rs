...
impl CutlassExpertsWeights {
    ...
    pub(super) fn forward_impl(
        ...
        inference_quant::moe::cutlass_fused_moe(
            forward.xs_flat,
            &self.w.gate_up,
            &self.w.down,
            forward.topk_ids,
...
pub(super) struct CutileFp8ExpertsWeights {
    w: inference_quant::cutile::CutileFp8MoeWeights,
}
...
impl CutileFp8ExpertsWeights {
    ...
    pub(super) fn try_from_checkpoint(
        ...
        if block.as_slice() != [FP8_MOE_GROUP, FP8_MOE_GROUP]
            || *activation_scheme != Some(inference_quant::Fp8ActivationScheme::Dynamic)
            || comm.world_size() > 1
        ...
        if !inference_quant::cutile::device_supported(dev)
        ...
        let w = inference_quant::cutile::CutileFp8MoeWeights::new(
            gate_up,
            gate_up_scales,
            down,
            down_scales,
        ...
        inference_quant::cutile::register_moe_fp8_shape(&w);
        inference_quant::log::once_log_info(
    ...
    pub(super) fn forward_impl(
        ...
        inference_quant::cutile::cutile_fused_moe_fp8(
            &xs,
            &self.w,
            forward.topk_ids,
            forward.topk_weights,
...
impl CutileExpertsWeights {
    pub(super) fn from_checkpoint(ckpt: &ExpertCheckpoint) -> Result<CutileExpertsWeights> {
        ...
        inference_quant::cutile::register_moe_shape(
            w.gate_up.clone(),
            w.down.clone(),
...
impl FastExpertsWeights {
    pub(super) fn from_weight_source(
        ...
        let load = |key: &str,
         ...
         -> Result<Arc<dyn QuantMethod>> {
            let load_device = inference_quant::weight_source_load_device(&predicate_vb);
            let layer = source
    ...
    pub(super) fn load_unquantized(
        ...
        let keys = QuantizedExpertKeys::new(&experts_vb.prefix());
        let shard = (comm.world_size() == 1).then(inference_quant::Shard::default);
...
impl FastExpertsWeights {
    ...
    pub(super) fn load_prequantized(
        ...
        #[cfg(feature = "cutile")]
        inference_quant::cutile::register_nvfp4_routing(
            vb.device(),
            cfg.num_experts,
            cfg.num_experts_per_tok,
        )?;
...
impl CutileExpertsWeights {
    pub(super) fn forward_impl(
        ...
        let cfg = inference_quant::cutile::get_default_config(num_tokens, num_experts);
        ...
        inference_quant::moe::cuda::moe_sum_bf16(&ic3, num_tokens, topk, dev)
    }
    ...
    fn forward_lora(&self, forward: &MoEForward, config: MoEForwardConfig) -> Result<Tensor> {
        ...
        let gate_up = inference_quant::cutile::cutile_grouped_gemm(
            forward.xs_flat,
            &self.w.gate_up,
            &sids,
            &eids,
        ...
        let down = inference_quant::cutile::cutile_grouped_gemm(
        ...
        inference_quant::moe::cuda::moe_sum_bf16(
...
impl FastExpertsWeights {
    ...
    fn forward_decode_lora(
        ...
        let weights = inference_quant::IndexedMoeLoraWeights::new(&gate_qt, &up_qt, &down_qt);
        let routing = inference_quant::IndexedMoeRouting::new(
            ids,
            forward.shape.num_tokens,
            config.num_experts_per_tok,
            config.num_experts,
        ...
        let Some(decode) = inference_quant::IndexedMoeLoraDecode::new(weights, routing)? else {
            return Ok(None);
        };
        ...
        let output = inference_quant::moe_weighted_reduce_flat_same_dtype(
            &down,
            forward.topk_weights,
            forward.shape.num_tokens,
            config.num_experts_per_tok,
    ...
    pub(super) fn forward_decode(
        ...
        let act_type = match config.act {
            Activation::GeluPytorchTanh => inference_quant::ACT_GELU_PYTORCH_TANH,
            Activation::Silu | Activation::Swish => inference_quant::ACT_SILU,
            _ => return Ok(None),
        };

        // SAFETY: tw_ptr is a valid device pointer obtained from the topk_weights tensor above.
        let result = unsafe {
            inference_quant::indexed_moe_fused_decode(
                &gate_qt,
                &up_qt,
                &down_qt,
                forward.xs_flat,
    ...
    pub(super) fn forward_grouped(
        ...
        let (expert_bounds, sorted_token_ids, sorted_source_ids) =
            inference_quant::moe_dispatch_build(
                ti_u32_slice,
                total_assignments,
                num_experts,
                topk,
...
mod immediate_isq_tests {
    ...
    fn experts_vb() -> ShardedVarBuilder {
        inference_quant::ShardedSafeTensors::wrap(HashMap::new(), DType::F32, Device::Cpu)
...
mod nvfp4_tests {
    ...
    use inference_quant::{Nvfp4ActivationMode, Nvfp4Layer, Nvfp4LayerParts};
