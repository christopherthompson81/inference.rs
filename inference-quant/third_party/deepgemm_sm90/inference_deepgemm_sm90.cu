...
#include "inference_deepgemm_sm90.h"
...
constexpr uint32_t kBlockSize = INFERENCE_RS_DEEPGEMM_BLOCK_SIZE;
constexpr uint32_t kActivationScaleMAlignment =
    INFERENCE_RS_DEEPGEMM_ACTIVATION_SCALE_M_ALIGNMENT;
...
static_assert(std::is_standard_layout_v<InferenceDeepGemmPlan>);
static_assert(offsetof(InferenceDeepGemmPlan, workspace_bytes) == 56);
static_assert(offsetof(InferenceDeepGemmPlan, cache_key) == 64);
static_assert(sizeof(InferenceDeepGemmPlan) == 72);
static_assert(std::is_standard_layout_v<InferenceDeepGemmPrepared>);
static_assert(offsetof(InferenceDeepGemmPrepared, function) == 72);
static_assert(sizeof(InferenceDeepGemmPrepared) == 80);
...
bool useWaveBalancedSmallMSwapAB(uint32_t m, uint32_t n, uint32_t k,
  ...
  return blocks <= sm_count && blocks * kWaveBalancedMinWaveDivisor >= sm_count &&
         smem_bytes > 0 &&
         smem_bytes <= static_cast<int>(inference::deepgemm_official::kSmemCapacity);
}
...
inference::deepgemm_official::GemmConfig productionOfficialConfig(
    uint32_t m, uint32_t n, uint32_t k, uint32_t sm_count) {
  const auto heuristic =
      inference::deepgemm_official::getBestGemmConfig(m, n, k, sm_count);
  for (const auto& tuning : kOfficialSmallMTunings) {
    if (m > tuning.max_m || n != tuning.n || k != tuning.k) {
      continue;
    }
    const auto tuned = inference::deepgemm_official::getGemmConfig(
...
KernelFamily planKernelFamily(const InferenceDeepGemmPlan& plan) {
  if ((plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_OFFICIAL_1D2D) != 0) {
    return KernelFamily::Official1D2D;
  }
  if ((plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB) != 0 &&
...
bool pdlRequested() {
  const char* value = std::getenv("INFERENCE_RS_DEEPGEMM_PDL");
...
uint64_t planCacheKey(const InferenceDeepGemmPlan& plan) {
...
bool samePlan(const InferenceDeepGemmPlan& lhs, const InferenceDeepGemmPlan& rhs) {
...
int32_t cudaFailure(const char* operation, cudaError_t status) {
  ...
  return INFERENCE_RS_DEEPGEMM_CUDA_ERROR;
}
...
int32_t currentContext(CUcontext* context) {
  ...
  if (*context == nullptr) {
    ...
    return INFERENCE_RS_DEEPGEMM_UNAVAILABLE;
  }
  return INFERENCE_RS_DEEPGEMM_SUCCESS;
}
...
int32_t createPlan(uint32_t m, uint32_t n, uint32_t k, KernelFamily family,
  ...
  if (plan == nullptr || m == 0 || n == 0 || k == 0 || n % kBlockSize != 0 ||
    ...
    return INFERENCE_RS_DEEPGEMM_INVALID_ARGUMENT;
  }

  CUcontext context = nullptr;
  int32_t status = currentContext(&context);
  if (status != INFERENCE_RS_DEEPGEMM_SUCCESS) {
    return status;
  }
  CUdevice device = 0;
  ...
  if (family == KernelFamily::Official1D2D) {
    ...
    if (config.valid()) {
      flags |= INFERENCE_RS_DEEPGEMM_PLAN_OFFICIAL_1D2D;
      if (config.multicast_on_a()) {
        flags |= INFERENCE_RS_DEEPGEMM_PLAN_MULTICAST_ON_A;
      }
      if (pdlRequested()) {
        flags |= INFERENCE_RS_DEEPGEMM_PLAN_PDL;
      }
    ...
    flags |= INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB;
    ...
    if (swap_ab) {
      flags |= INFERENCE_RS_DEEPGEMM_PLAN_SWAP_AB;
    }
...
int32_t validatePreparedLaunch(const InferenceDeepGemmPrepared* prepared, uint32_t m) {
  ...
  if ((plan->flags & INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB) != 0 && m != plan->m) {
...
int32_t launchPrequantizedGemm(const InferenceDeepGemmPrepared* prepared, uint32_t m,
  ...
  const bool official = (plan->flags & INFERENCE_RS_DEEPGEMM_PLAN_OFFICIAL_1D2D) != 0;
  const bool swap_ab = (plan->flags & INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB) != 0;
  if (official) {
    launchOfficialGemm(
        ...
        (plan->flags & INFERENCE_RS_DEEPGEMM_PLAN_MULTICAST_ON_A) != 0,
        (plan->flags & INFERENCE_RS_DEEPGEMM_PLAN_PDL) != 0,
...
int32_t classifyException(const std::exception& error) {
  ...
  if (last_error.find("nvcc") != std::string::npos) {
    return INFERENCE_RS_DEEPGEMM_COMPILE_ERROR;
  }
  if (last_error.find("CUDA") != std::string::npos ||
      last_error.find("cuda") != std::string::npos) {
    return INFERENCE_RS_DEEPGEMM_CUDA_ERROR;
  }
  return INFERENCE_RS_DEEPGEMM_INTERNAL_ERROR;
}

}  // namespace

extern "C" const char* inference_deepgemm_sm90_error_string(int32_t status) {
  switch (status) {
    case INFERENCE_RS_DEEPGEMM_SUCCESS:
      return "success";
    case INFERENCE_RS_DEEPGEMM_UNAVAILABLE:
      return "provider unavailable";
    case INFERENCE_RS_DEEPGEMM_INVALID_ARGUMENT:
      return "invalid argument";
    case INFERENCE_RS_DEEPGEMM_NOT_PREPARED:
      return "plan not prepared";
    case INFERENCE_RS_DEEPGEMM_WORKSPACE_TOO_SMALL:
      return "workspace too small";
    case INFERENCE_RS_DEEPGEMM_CAPTURE_ACTIVE:
      return "preparation attempted during CUDA graph capture";
    case INFERENCE_RS_DEEPGEMM_COMPILE_ERROR:
      return "runtime compilation failed";
    case INFERENCE_RS_DEEPGEMM_CUDA_ERROR:
      return "CUDA operation failed";
    case INFERENCE_RS_DEEPGEMM_INTERNAL_ERROR:
...
extern "C" const char* inference_deepgemm_sm90_last_error() {
  return last_error.c_str();
}

extern "C" int32_t inference_deepgemm_sm90_plan(uint32_t m, uint32_t n, uint32_t k,
                                                 InferenceDeepGemmPlan* plan) {
  last_error.clear();
  try {
...
#ifdef INFERENCE_RS_DEEPGEMM_ENABLE_LEGACY_DIAGNOSTICS
extern "C" int32_t inference_deepgemm_sm90_plan_legacy_for_test(
...
extern "C" int32_t inference_deepgemm_sm90_prepare(const InferenceDeepGemmPlan* plan,
  ...
  try {
    ...
    if (capture_status != CU_STREAM_CAPTURE_STATUS_NONE) {
      ...
      return INFERENCE_RS_DEEPGEMM_CAPTURE_ACTIVE;
    }
...
extern "C" int32_t inference_deepgemm_sm90_gemm(
    const InferenceDeepGemmPrepared* prepared, uint32_t m, const void* activation_bf16,
  ...
  try {
    ...
    if (workspace_bytes < plan->workspace_bytes) {
      ...
      return INFERENCE_RS_DEEPGEMM_WORKSPACE_TOO_SMALL;
    }
...
extern "C" int32_t inference_deepgemm_sm90_gemm_prequantized(
    const InferenceDeepGemmPrepared* prepared, uint32_t m, const void* activation_e4m3,
