...
#ifndef INFERENCE_RS_DEEPGEMM_ENABLE_LEGACY_DIAGNOSTICS
...
bool benchmarkFamily(const Shape& shape, const char* family, bool legacy,
  ...
  const int32_t plan_status =
      legacy ? inference_deepgemm_sm90_plan_legacy_for_test(kM, shape.n, shape.k, &plan)
             : inference_deepgemm_sm90_plan(kM, shape.n, shape.k, &plan);
  if (!checkProvider(plan_status, "plan") ||
      !checkProvider(inference_deepgemm_sm90_prepare(&plan, include_dir, stream, &prepared),
                     "prepare")) {
    return false;
  }

  auto launch = [&]() {
    return checkProvider(inference_deepgemm_sm90_gemm_prequantized(
