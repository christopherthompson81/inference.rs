...
mod cuda_bench {
    ...
    fn iterations() -> usize {
        env::var("INFERENCE_RS_LORA_BENCH_ITERS")
            .ok()
    ...
    fn reference_iterations() -> usize {
        env::var("INFERENCE_RS_LORA_BENCH_REFERENCE_ITERS")
            .ok()
            .and_then(|value| value.parse().ok())
    ...
    fn samples(iterations: usize) -> usize {
        env::var("INFERENCE_RS_LORA_BENCH_SAMPLES")
            .ok()
            .and_then(|value| value.parse().ok())
    ...
    fn benchmark_ranks(profile: &str) -> Result<Vec<usize>> {
        if let Ok(ranks) = env::var("INFERENCE_RS_LORA_BENCH_RANKS") {
            let ranks = ranks
                .split(',')
                .map(|rank| {
    ...
    fn case_is_selected(name: &str) -> bool {
        env::var("INFERENCE_RS_LORA_BENCH_CASES")
            .ok()
            .is_none_or(|selected| {
                selected
                    .split(',')
    ...
    fn benchmark_cases(profile: &str) -> Result<Vec<BenchCase>> {
        ...
        if let Ok(selected) = env::var("INFERENCE_RS_LORA_BENCH_CASES") {
            cases.retain(|case| {
                selected
                    .split(',')
                    .map(str::trim)
