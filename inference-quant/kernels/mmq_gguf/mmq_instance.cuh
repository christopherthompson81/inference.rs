...
// One .cu per quant type keeps the kernel build parallel; each includes this and instantiates its type.
...
static void instantiate_mmq(float *tmp_fixup, const mmq_args &args,
  ...
  const uint3 blocks_per_ne00_fd = init_fastdiv_values(
      args.ncols_x / ggml_cuda_type_traits<type>::qk);
  ...
  CUDA_SET_SHARED_MEMORY_LIMIT((mul_mat_q<type, mmq_x, false>),
                               nbytes_shared);
  CUDA_SET_SHARED_MEMORY_LIMIT((mul_mat_q<type, mmq_x, true>),
                               nbytes_shared);
...
static void launch_mmq_case(float *tmp_fixup, const mmq_args &args,
  ...
  for (int mmq_x = 8; mmq_x <= mmq_x_max && ntiles_x_best > 1; mmq_x += 8) {
    ...
    if (mmq_x % granularity != 0)
      continue;
    const size_t nbs = mmq_get_nbytes_shared<type>(
        mmq_x, mmq_y, cc, warp_size_host, nwarps);
    if (nbs > smpbo)
      continue;
...
template <ggml_type type>
static void launch_mmq_gguf(void *tmp_fixup_ptr, const void *x,
                                     const void *y_q8_1_mmq, void *dst,
                                     int64_t ncols_x, int64_t nrows_x,
...
#define DEFINE_MMQ_GGUF_INSTANCE(name, type)                                    \
  extern "C" void launch_mmq_gguf_##name(                                     \
    ...
    launch_mmq_gguf<type>(tmp_fixup_ptr, x, y_q8_1_mmq, dst, ncols_x,         \
                          nrows_x, ncols_y, stride_row_x, stride_col_dst, cc, \
                          nsm, smpbo, warp_size_host, type_dst, stream);      \
  }                                                                           \
