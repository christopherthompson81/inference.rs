...
bool valid_dtype(int32_t dtype) {
  return dtype == INFERENCE_RS_NVFP4_BF16 || dtype == INFERENCE_RS_NVFP4_F16;
}

bool valid_kernel(int32_t kernel) {
  return kernel == INFERENCE_RS_NVFP4_PREFILL ||
         kernel == INFERENCE_RS_NVFP4_DECODE_DP32;
}

bool valid_context(const inference_nvfp4_context *context) {
  return context && context->device >= 0 && context->sm_count > 0 &&
         valid_dtype(context->dtype) && valid_kernel(context->kernel);
}

bool valid_shape(const inference_nvfp4_shape *shape) {
...
make_arguments(const inference_nvfp4_launch &launch) {
...
template <class Kernel>
int prepare_kernel(const inference_nvfp4_context &context,
...
template <class Kernel>
int workspace_size(const inference_nvfp4_context &context,
                   const inference_nvfp4_shape &shape, size_t *bytes) {
  using Adapter = cutlass::gemm::device::GemmUniversalAdapter<Kernel>;
  inference_nvfp4_launch launch{};
  launch.shape = shape;
...
template <class Kernel> int gemm(const inference_nvfp4_launch &launch) {
  using Adapter = cutlass::gemm::device::GemmUniversalAdapter<Kernel>;
  auto arguments = make_arguments<Kernel>(launch);
  auto status = Adapter::can_implement(arguments);
  if (status != cutlass::Status::kSuccess)
...
using namespace inference_nvfp4;

extern "C" const char *inference_nvfp4_error_string(int status) {
...
extern "C" int inference_nvfp4_prepare(int32_t device, int32_t dtype,
                                       int32_t kernel,
                                       inference_nvfp4_resources *resources) {
  ...
  inference_nvfp4_context context{device, props.multiProcessorCount, dtype,
                                  kernel};
  if (kernel == INFERENCE_RS_NVFP4_DECODE_DP32)
    return dtype == INFERENCE_RS_NVFP4_BF16
...
extern "C" int
inference_nvfp4_workspace_size(const inference_nvfp4_context *context,
                               const inference_nvfp4_shape *shape,
                               size_t *bytes) {
  if (!valid_context(context) || !valid_shape(shape) || !bytes)
    return -static_cast<int>(cudaErrorInvalidValue);
  if (context->kernel == INFERENCE_RS_NVFP4_DECODE_DP32)
    return context->dtype == INFERENCE_RS_NVFP4_BF16
...
extern "C" int inference_nvfp4_gemm(const inference_nvfp4_launch *launch) {
  if (!launch || !valid_context(&launch->context) ||
    ...
    return -static_cast<int>(cudaErrorInvalidValue);
  if (launch->context.kernel == INFERENCE_RS_NVFP4_DECODE_DP32)
    return launch->context.dtype == INFERENCE_RS_NVFP4_BF16
...
extern "C" int inference_nvfp4_scale_bytes(int32_t rows, int32_t k,
                                           size_t *bytes) {
  size_t cols = 0, padded_cols = 0;
  return scale_sizes(rows, k, &cols, &padded_cols, bytes);
}

extern "C" int inference_nvfp4_swizzle_host(const void *source, void *dest,
...
extern "C" int inference_nvfp4_swizzle_cuda(const void *source, void *dest,
                                            int32_t rows, int32_t k,
