...
extern "C" const char *inference_fp8_mma_error_string(int status) {
  switch (status) {
    case kOk: return "ok";
...
extern "C" int inference_fp8_mma_gemv(const void *w, const void *xq, const float *xs,
...
extern "C" int inference_fp8_mma_quantize_bf16(const void *x, void *xq, float *xs, int rows,
