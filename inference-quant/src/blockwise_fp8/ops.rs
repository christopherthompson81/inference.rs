...
fn check_cutlass_status(operation: &str, status: i32) -> Result<()> {
    ...
    let message = unsafe {
        let message = ffi::inference_cutlass_fp8_error_string(status);
        if message.is_null() {
...
fn cutlass_workspace_size(key: CutlassWorkspaceRequirementsKey) -> Result<usize> {
    ...
    let status = unsafe {
        ffi::inference_cutlass_fp8_blockwise_workspace_size(
            key.m,
            key.n,
            key.k,
            key.output_dtype,
...
pub(crate) fn fp8_quantize_activation_cutlass(input: &Tensor) -> Result<(Tensor, Tensor)> {
    ...
    let status = match input.dtype() {
        DType::F16 => {
            ...
            let status = unsafe {
                ffi::inference_fp8_quantize_activation_f16(
                    input_ptr as *const f16,
                    quantized_ptr as *mut F8E4M3,
                    scales_ptr as *mut f32,
                    rows_i32,
        ...
        DType::BF16 => {
            ...
            let (input_ptr, input_guard) =
                slice_ptr_on_stream(input, input_layout.start_offset(), &stream);
            let status = unsafe {
                ffi::inference_fp8_quantize_activation_bf16(
                    input_ptr as *const bf16,
                    quantized_ptr as *mut F8E4M3,
                    scales_ptr as *mut f32,
                    rows_i32,
...
fn fused_add_rms_norm_quantized_impl(
    ...
    #[cfg(all(feature = "cuda", has_blockwise_fp8_kernels))]
    {
        ...
        let (scales_ptr, scales_guard) = slice_ptr_mut_on_stream(&mut scales, 0, &stream);
        let status = unsafe {
            super::ffi::inference_fused_add_rms_norm_quantize_bf16(
                input_ptr as *const bf16,
        ...
        if status != 0 {
            let error = unsafe {
                CStr::from_ptr(super::ffi::inference_fused_rms_norm_fp8_error_string(
                    status,
                ))
            }
            .to_string_lossy();
...
fn launch_cutlass_gemm(context: CutlassGemm<'_>, output_ptr: u64) -> Result<()> {
    ...
    let status = unsafe {
        ffi::inference_cutlass_fp8_blockwise_gemm(
