...
#pragma unroll(4)
  ...
  for (short jj = 0; jj < NQ; ++jj) {
    ...
    // inference output layout is [b, n_heads, q_seq, DV] (heads before
...
// dispatch time. Output layout: [b, n_heads, q_seq, DV] (inference convention;
...
#pragma unroll(DV4 / NL)
  ...
  // Final write. Output layout [b, n_heads, q_seq, DV] (inference convention).
  if (sgitg == 0) {
    const float S = ss[0];
