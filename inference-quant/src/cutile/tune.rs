...
pub const TUNE_MODE_ENV: &str = "INFERENCE_RS_CUTILE_TUNE";
/// Overrides the directory holding the tuning records.
pub const TUNE_CACHE_ENV: &str = "INFERENCE_RS_CUTILE_TUNE_CACHE";
...
fn record_dir() -> PathBuf {
    ...
    if let Some(path) = std::env::var_os("XDG_CACHE_HOME") {
        return PathBuf::from(path).join("inference-rs").join(RECORD_DIR);
    }
    ...
    std::env::temp_dir().join("inference-rs").join(RECORD_DIR)
}
