...
SMOKE_EXAMPLES=(--example text_generation --example streaming --example multimodal_basic)
if [[ $tests -eq 1 ]]; then
    cargo nextest run --workspace "${TEST_TARGETS[@]}"
    # nextest does not run doctests
    cargo test --workspace --no-fail-fast --doc
    # --workspace keeps the same feature unification as the tests above, so no second copy of the crates gets built
    cargo build --workspace "${SMOKE_EXAMPLES[@]}"
fi
if [[ $cuda -eq 1 ]]; then
