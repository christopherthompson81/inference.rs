...
async fn run_report(args: ReportCommandArgs, token_source: &TokenSource) -> Result<()> {
    ...
    let report = build_uqff_report_from_artifacts(
        &resolved.artifacts,
        UqffReportOptions {
            generated_by: generated_by("inference uqff report"),
            base_model,
            repo_id,
        },
    )
    ...
    if write {
        let Some(path) = resolved.local_write_path else {
            return Err(anyhow!(
                "`inference uqff report --write` requires a local model path. Use `--json` for Hugging Face repos."
            ));
        };
...
async fn run_inspect(
    ...
    let inspection = inspect_uqff_artifacts(
        &resolved.artifacts,
        UqffReportOptions {
            generated_by: generated_by("inference uqff inspect"),
            base_model: None,
            repo_id: None,
        },
    )
