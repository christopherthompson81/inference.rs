...
// Managed installs live in ~/.inference-rs; canonicalize so symlinks resolve to the real install path.
...
fn print_unmanaged_uninstall_hint() {
    ...
    println!("`inference uninstall` only manages installer-managed installs.");
    println!("This binary is at {}.", exe.display());
    println!("Remove it with: cargo uninstall inference-cli");
}
...
pub fn run_update(version: Option<String>) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        println!("Updating inference.rs...");
        let cmd = format!("curl --proto '=https' --tlsv1.2 -sSf {INSTALL_SH_URL} | sh");
        let mut c = std::process::Command::new("sh");
        c.arg("-c")
            .arg(&cmd)
            .env("INFERENCE_RS_INSTALL_YES", "1")
            .env("INFERENCE_RS_INSTALL_IGNORE_FFMPEG", "1");
        if let Some(v) = &version {
            c.env("INFERENCE_RS_INSTALL_TAG", v);
        }
...
pub fn run_uninstall(yes: bool) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        println!(
            "Delete {} and the inference launcher on your PATH manually.",
            pre.display()
        );
        Ok(())
    }
    ...
    #[cfg(unix)]
    {
        ...
        if let Some(bin) = bin_dir() {
            let inference_link = bin.join("inference-rs");
            let is_symlink = inference_link
                .symlink_metadata()
                .map(|metadata| metadata.file_type().is_symlink())
                .unwrap_or(false);
            if is_symlink {
                let _ = std::fs::remove_file(&inference_link);
            }
        ...
        println!(
            "Removed {} and symlinks. inference.rs uninstalled.",
            pre.display()
        );
