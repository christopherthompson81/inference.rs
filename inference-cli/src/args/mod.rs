//! CLI argument definitions for inference-cli
//!
...
mod tests {
    ...
    fn quantized_file_infers_gguf_and_local_root() {
        let filename = format!("inference-{}.GGUF", uuid::Uuid::new_v4());
    ...
    fn direct_gguf_shorthand_preserves_a_cross_directory_projector() {
        let root = PathBuf::from(format!(".inference-rs-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(root.join("weights")).unwrap();
