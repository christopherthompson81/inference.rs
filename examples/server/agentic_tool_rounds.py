...
1. Start the server with MCP tools and max_tool_rounds:
   inference_rs serve -p 1234 --mcp-config examples/mcp-simple-config.json --max-tool-rounds 5 -m Qwen/Qwen3-4B

   Or with built-in agent tools:
   inference_rs serve -p 1234 --agent --max-tool-rounds 5 -m Qwen/Qwen3-4B
...
def main():
    ...
    try:
        ...
        print(
            "inference_rs serve -p 1234 --mcp-config examples/mcp-simple-config.json "
