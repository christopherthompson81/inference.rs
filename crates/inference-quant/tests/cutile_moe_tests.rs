...
fn run_cutile_case(
    ...
    ids: Vec<u32>,
    weights: Vec<f32>,
    act: inference_quant::moe::cuda::GatedAct,
    tol: f32,
) -> Result<()> {
    ...
    let act_fn = match act {
        inference_quant::moe::cuda::GatedAct::GeluTanh => gelu_tanh as fn(f32) -> f32,
        inference_quant::moe::cuda::GatedAct::Silu => silu as fn(f32) -> f32,
    };
    ...
    let got = inference_quant::moe::cuda::moe_sum_bf16(&ic3, num_tokens, topk, dev)?
        .to_dtype(DType::F32)?
        .to_device(&cpu)?
        .flatten_all()?
        .to_vec1::<f32>()?;
