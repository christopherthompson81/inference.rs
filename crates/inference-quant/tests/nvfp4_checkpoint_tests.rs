...
#[test]
#[ignore = "requires scripts/testgen/fetch_nvfp4_test_fixtures.py output in INFERENCE_RS_NVFP4_FIXTURE_DIR"]
...
fn packed_nvfp4_preserves_output_shards_and_row_global_scales() -> Result<()> {
    use inference_quant::{ColumnParallelLayer, Comm, Id, LoraLayerRegistry};
    use std::sync::Arc;
...
fn packed_nvfp4_falls_back_for_distinct_calibrated_input_scales() -> Result<()> {
    use inference_quant::{ColumnParallelLayer, Comm, Id};
    use std::sync::Arc;
...
fn packed_nvfp4_keeps_exclusions_and_output_layout_fallbacks() -> Result<()> {
    use inference_quant::{ColumnParallelLayer, Comm, Id, PackedOutputLayout};
    use std::sync::Arc;
