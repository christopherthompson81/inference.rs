...
const INFERENCE_RS_NO_NCCL: &str = "INFERENCE_RS_NO_NCCL";
...
pub fn get_global_tp_size_from_devices() -> Result<usize> {
    ...
    #[cfg(all(feature = "cuda", feature = "nccl"))]
    {
        ...
        if let Ok(x) = std::env::var(INFERENCE_RS_MN_LOCAL_WORLD_SIZE) {
            use std::str::FromStr;
            Ok(usize::from_str(&x).expect("Not a number for INFERENCE_RS_MN_LOCAL_WORLD_SIZE!"))
        } else {
...
pub fn use_nccl() -> bool {
    if std::env::var(INFERENCE_RS_NO_NCCL).is_ok_and(|x| x == "1") {
        return false;
    }
...
fn nccl_world_size() -> Option<usize> {
    if let Some(global_world_size) = parse_env_usize(INFERENCE_RS_MN_GLOBAL_WORLD_SIZE) {
        return Some(global_world_size);
    }
    if let Some(local_world_size) = parse_env_usize(INFERENCE_RS_MN_LOCAL_WORLD_SIZE) {
