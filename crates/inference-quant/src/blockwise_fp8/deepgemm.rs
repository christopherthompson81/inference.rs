...
fn deepgemm_cache_root() -> PathBuf {
    if let Some(path) = std::env::var_os("INFERENCE_RS_DEEPGEMM_CACHE_DIR") {
        return PathBuf::from(path);
    }
    if let Some(path) = std::env::var_os("XDG_CACHE_HOME") {
        return PathBuf::from(path).join("inference/deepgemm-sm90");
    }
    if let Some(path) = std::env::var_os("HOME") {
        return PathBuf::from(path).join(".cache/inference/deepgemm-sm90");
    }
    let uid = unsafe { libc::geteuid() };
    std::env::temp_dir().join(format!("inference-deepgemm-sm90-{uid}"))
}
...
fn deepgemm_status_message(status: i32) -> Cow<'static, str> {
    unsafe {
        let detail = ffi::inference_deepgemm_sm90_last_error();
        if !detail.is_null() {
        ...
        let message = ffi::inference_deepgemm_sm90_error_string(status);
        if message.is_null() {
        ...
        let prepare_inner = || -> Result<Prepared> {
            ...
            for m in (1..=DECODE_MAX_M).chain(PREFILL_M_VALUES) {
                ...
                let mut status = unsafe {
                    ffi::inference_deepgemm_sm90_prepare(
                        &plan,
                        std::ptr::null(),
                        stream,
                        prepared.as_mut_ptr(),
