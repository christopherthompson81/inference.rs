...
unsafe extern "C" {
    fn inference_nvfp4_error_string(status: i32) -> *const c_char;
    fn inference_nvfp4_prepare(
        device: i32,
        dtype: i32,
        kernel: i32,
        resources: *mut Resources,
    ) -> i32;
    fn inference_nvfp4_workspace_size(
        context: *const Context,
        shape: *const Problem,
        bytes: *mut usize,
    ) -> i32;
    fn inference_nvfp4_gemm(launch: *const Launch) -> i32;
    fn inference_nvfp4_scale_bytes(rows: i32, k: i32, bytes: *mut usize) -> i32;
    fn inference_nvfp4_swizzle_cuda(
        source: *const c_void,
...
fn check_status(operation: &str, status: i32) -> Result<()> {
    ...
    let message = unsafe { inference_nvfp4_error_string(status) };
    ...
    for kernel in [PREFILL_KERNEL, DECODE_KERNEL] {
        for dtype in 0..DTYPE_COUNT {
            ...
            check_status("kernel preparation", unsafe {
                inference_nvfp4_prepare(ordinal, dtype as i32, kernel, &mut resources)
            })?;
...
fn scale_bytes(rows: usize, k: usize) -> Result<usize> {
    ...
    check_status("scale size query", unsafe {
        inference_nvfp4_scale_bytes(i32::try_from(rows)?, i32::try_from(k)?, &mut bytes)
    })?;
    Ok(bytes)
}
...
impl State {
    ...
    pub(crate) fn forward(
        ...
        check_status("workspace query", unsafe {
            inference_nvfp4_workspace_size(&context, &shape, &mut workspace_bytes)
        })?;
        ...
        macro_rules! run {
            ($dtype:ty) => {{
                ...
                check_status("matmul", unsafe { inference_nvfp4_gemm(&launch) })?;
                drop(guard);
                Tensor::from((
                    Storage::Cuda(CudaStorage::wrap_cuda_slice(output, device.clone())),
