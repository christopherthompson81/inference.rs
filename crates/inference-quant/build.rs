...
fn main() -> Result<(), String> {
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        let header_hash_arg = format!(
            "-DINFERENCE_RS_QUANT_CUDA_HEADER_HASH=0x{:016x}",
        ...
        if deepgemm_fp8_sm90 {
            ...
            deepgemm_source_hash = deepgemm_jit_bundle::write(
                &out_dir.join("deepgemm_jit_headers.rs"),
        ...
        println!("cargo:rustc-env=INFERENCE_RS_DEEPGEMM_SOURCE_HASH={deepgemm_source_hash:016x}");
        ...
        let out_file = if target.contains("msvc") {
            // Windows case
            out_dir.join("inferencequant.lib")
        } else {
        ...
        if deepgemm_fp8_sm90 {
            let deepgemm_source_hash_arg =
                format!("-DINFERENCE_RS_DEEPGEMM_SOURCE_HASH=0x{deepgemm_source_hash:016x}");
            let deepgemm_build_dir = cuda_build_dir(&out_dir, "deepgemm");
            let mut deepgemm_builder = cuda_kernel_builder(&deepgemm_build_dir)
                .source_files(["third_party/deepgemm_sm90/inference_deepgemm_sm90.cu"])
            ...
            deepgemm_builder
                .build_lib(prepare_cuda_archive(out_dir.join("libinferencedeepgemm.a")))
        ...
        if nvfp4_cutlass_supported_for_build(
            ...
            nvfp4_builder
                .build_lib(prepare_cuda_archive(out_dir.join("libinferencenvfp4.a")))
        ...
        println!("cargo:rustc-env=INFERENCE_RS_BUILD_CUDA_VERSION={major}.{minor}");
        println!(
            "cargo:rustc-env=INFERENCE_RS_BUILD_CUDA_VERSION_CODE={}",
            major * 100 + minor
        );
    ...
    #[cfg(feature = "metal")]
    {
        inference_metal_compile::compile_metallibs(&QUANT_METAL_SOURCE_SET)
    }
