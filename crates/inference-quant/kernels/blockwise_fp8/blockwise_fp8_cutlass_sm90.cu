...
extern "C" const char *inference_cutlass_fp8_error_string(int status) {
  if (status < 0) {
...
extern "C" int inference_cutlass_fp8_blockwise_prepare() {
  int status = inference::fp8::prepare_kernel<
      inference::fp8::LargeMGemm<cutlass::half_t>>();
  if (status != 0) {
    return status;
  }
  status = inference::fp8::prepare_kernel<
      inference::fp8::SmallMGemm<cutlass::half_t>>();
  if (status != 0) {
    return status;
  }
  status = inference::fp8::prepare_kernel<
      inference::fp8::SmallMCooperativeGemm<cutlass::half_t>>();
  if (status != 0) {
    return status;
  }
  status = inference::fp8::prepare_kernel<
      inference::fp8::SmallMStreamKGemm<cutlass::half_t>>();
  if (status != 0) {
    return status;
  }
  status = inference::fp8::prepare_kernel<
      inference::fp8::LargeMGemm<cutlass::bfloat16_t>>();
  if (status != 0) {
    return status;
  }
  status = inference::fp8::prepare_kernel<
      inference::fp8::SmallMGemm<cutlass::bfloat16_t>>();
  if (status != 0) {
    return status;
  }
  status = inference::fp8::prepare_kernel<
      inference::fp8::SmallMCooperativeGemm<cutlass::bfloat16_t>>();
  if (status != 0) {
    return status;
  }
  return inference::fp8::prepare_kernel<
      inference::fp8::SmallMStreamKGemm<cutlass::bfloat16_t>>();
}

extern "C" int inference_cutlass_fp8_blockwise_workspace_size(
    int m, int n, int k, int output_dtype, int sm_count, size_t *bytes) {
  if (output_dtype == INFERENCE_RS_CUTLASS_FP8_OUTPUT_F16) {
    return inference::fp8::dispatch_workspace_size<cutlass::half_t>(
        m, n, k, sm_count, bytes);
  }
  if (output_dtype == INFERENCE_RS_CUTLASS_FP8_OUTPUT_BF16) {
    return inference::fp8::dispatch_workspace_size<cutlass::bfloat16_t>(
        m, n, k, sm_count, bytes);
  }
  return -static_cast<int>(cudaErrorInvalidValue);
}

extern "C" int inference_cutlass_fp8_blockwise_gemm(
  ...
  if (output_dtype == INFERENCE_RS_CUTLASS_FP8_OUTPUT_F16) {
    return inference::fp8::dispatch_gemm<cutlass::half_t>(
        activation, weight, activation_scales, weight_scales, output, m, n, k,
        workspace, workspace_bytes, sm_count, cuda_stream);
  }
  if (output_dtype == INFERENCE_RS_CUTLASS_FP8_OUTPUT_BF16) {
    return inference::fp8::dispatch_gemm<cutlass::bfloat16_t>(
        activation, weight, activation_scales, weight_scales, output, m, n, k,
        workspace, workspace_bytes, sm_count, cuda_stream);
  }
  return -static_cast<int>(cudaErrorInvalidValue);
