...
constexpr uint32_t kScaleBlock = INFERENCE_RS_DEEPGEMM_BLOCK_SIZE;
constexpr uint32_t kScaleStrideM = INFERENCE_RS_DEEPGEMM_ACTIVATION_SCALE_M_ALIGNMENT;
...
bool providerOk(int32_t status, const char* operation) {
  ...
  std::fprintf(stderr, "%s: %s: %s\n", operation,
               inference_deepgemm_sm90_error_string(status),
               inference_deepgemm_sm90_last_error());
  return false;
}

bool selectionOk(uint32_t m, uint32_t n, uint32_t k, bool expect_small_m) {
  InferenceDeepGemmPlan plan{};
  if (!providerOk(inference_deepgemm_sm90_plan(m, n, k, &plan), "selection plan")) {
    return false;
  }
  const bool small_m = (plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB) != 0;
...
bool waveBalancedSelectionOk() {
  InferenceDeepGemmPlan plan{};
  if (!providerOk(inference_deepgemm_sm90_plan(8, kGenericN, kGenericK, &plan),
                  "wave-balanced plan")) {
    return false;
  }
...
bool officialTileOk(uint32_t m, uint32_t block_m, uint32_t cluster_size) {
  InferenceDeepGemmPlan plan{};
  if (!providerOk(inference_deepgemm_sm90_plan(m, kGateN, kGateK, &plan),
                  "official tile plan")) {
    return false;
  }
  if ((plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_OFFICIAL_1D2D) == 0 ||
      (plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB) != 0 ||
...
int main(int argc, char** argv) {
  ...
  InferenceDeepGemmPlan plan{};
  if (!providerOk(inference_deepgemm_sm90_plan(kM, kN, kK, &plan), "plan")) {
    return 1;
  }
  if ((plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_SMALL_M_SWAP_AB) == 0 ||
      (plan.flags & INFERENCE_RS_DEEPGEMM_PLAN_OFFICIAL_1D2D) != 0 || plan.block_n != 8 ||
      plan.workspace_bytes == 0) {
    std::fprintf(stderr, "unexpected small-M plan\n");
    return 1;
  }

  InferenceDeepGemmPlan captured_plan{};
  InferenceDeepGemmPrepared captured_prepared{};
  if (!providerOk(inference_deepgemm_sm90_plan(2, 128, 128, &captured_plan), "captured plan")) {
    return 1;
  }
  ...
  int32_t capture_status =
      inference_deepgemm_sm90_prepare(&captured_plan, include_dir, stream,
                                      &captured_prepared);
  if (capture_status != INFERENCE_RS_DEEPGEMM_CAPTURE_ACTIVE) {
  ...
  if (!providerOk(inference_deepgemm_sm90_prepare(&plan, include_dir, stream, &prepared),
                  "prepare")) {
    return 1;
  }
  ...
  if (!providerOk(inference_deepgemm_sm90_gemm(
  ...
  int32_t invalid_stride_status = inference_deepgemm_sm90_gemm_prequantized(
      &prepared, plan.m, activation_fp8, activation_scales, plan.m, weight, weight_scale,
      output, stream);
  if (invalid_stride_status != INFERENCE_RS_DEEPGEMM_INVALID_ARGUMENT) {
    std::fprintf(stderr, "invalid activation scale stride returned %d\n", invalid_stride_status);
    return 1;
  }
  if (!providerOk(inference_deepgemm_sm90_gemm_prequantized(
  ...
  if (!cudaOk(cudaMemcpyAsync(activation_fp8, host_activation_fp8.data(),
      ...
      !providerOk(inference_deepgemm_sm90_gemm_prequantized(
  ...
  if (!cudaOk(cudaStreamBeginCapture(stream, cudaStreamCaptureModeThreadLocal),
              "graph begin") ||
      !providerOk(inference_deepgemm_sm90_gemm(
                      &prepared, plan.m, activation, weight, weight_scale, output, workspace,
                      plan.workspace_bytes, stream),
                  "captured gemm") ||
      !providerOk(inference_deepgemm_sm90_gemm_prequantized(
