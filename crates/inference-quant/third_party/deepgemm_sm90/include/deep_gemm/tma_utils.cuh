...
PFN_cuTensorMapEncodeTiled_v12000 get_cuTensorMapEncodeTiled() {
  ...
  auto status = cudaGetDriverEntryPointByVersion("cuTensorMapEncodeTiled",
                                                 &cuTensorMapEncodeTiled_ptr, 12000,
                                                 cudaEnableDefault, &driver_status);
#else
  auto status = cudaGetDriverEntryPoint("cuTensorMapEncodeTiled", &cuTensorMapEncodeTiled_ptr,
                                        cudaEnableDefault, &driver_status);
#endif
  ...
  if (driver_status != cudaDriverEntryPointSuccess || cuTensorMapEncodeTiled_ptr == nullptr) {
    throw std::runtime_error("cuTensorMapEncodeTiled is unavailable from the CUDA driver");
  }
...
CUtensorMap make_2d_tma_copy_desc(T* global_address, uint64_t gmem_dim[2], uint64_t stride_in_bytes,
  ...
  auto result =
      encode_func(&tensor_map, get_CUtensorMapDataType<typename std::remove_cv<T>::type>(), rank,
...
__device__ __forceinline__ void tma_copy(void const* desc_ptr, uint64_t* barrier_ptr,
                                         void* smem_ptr, int32_t const& crd_0,
                                         int32_t const& crd_1) {
  ...
  if constexpr (kNumTMAMulticast == 1) {
    ...
    cute::SM90_TMA_LOAD_MULTICAST_2D::copy(desc_ptr, barrier_ptr, (1 << kNumTMAMulticast) - 1,
