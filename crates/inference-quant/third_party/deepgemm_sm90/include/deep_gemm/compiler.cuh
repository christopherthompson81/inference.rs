...
#ifndef INFERENCE_RS_DEEPGEMM_SOURCE_HASH
#error "INFERENCE_RS_DEEPGEMM_SOURCE_HASH must identify the bundled JIT sources"
#endif

#define INFERENCE_RS_DEEPGEMM_STRINGIFY_IMPL(value) #value
#define INFERENCE_RS_DEEPGEMM_STRINGIFY(value) INFERENCE_RS_DEEPGEMM_STRINGIFY_IMPL(value)

inline std::string getCacheVersion() {
  return "source-" INFERENCE_RS_DEEPGEMM_STRINGIFY(INFERENCE_RS_DEEPGEMM_SOURCE_HASH);
}

#undef INFERENCE_RS_DEEPGEMM_STRINGIFY
#undef INFERENCE_RS_DEEPGEMM_STRINGIFY_IMPL

inline std::filesystem::path getDefaultUserDir() {
  if (const char* override_dir = std::getenv("INFERENCE_RS_DEEPGEMM_CACHE_DIR");
      override_dir != nullptr && override_dir[0] != '\0') {
    return override_dir;
  }
  if (const char* xdg_cache = std::getenv("XDG_CACHE_HOME");
      xdg_cache != nullptr && xdg_cache[0] != '\0') {
    return std::filesystem::path(xdg_cache) / "inference" / "deepgemm-sm90";
  }
  if (const char* user_home = std::getenv("HOME"); user_home != nullptr && user_home[0] != '\0') {
    return std::filesystem::path(user_home) / ".cache" / "inference" / "deepgemm-sm90";
  }
#ifdef _WIN32
  return std::filesystem::temp_directory_path() / "inference-deepgemm-sm90";
#else
  return std::filesystem::temp_directory_path() /
         ("inference-deepgemm-sm90-" + std::to_string(static_cast<uint64_t>(geteuid())));
#endif
}
...
inline std::string getNvccCompiler() {
  if (const char* nvcc = std::getenv("INFERENCE_RS_DEEPGEMM_NVCC");
