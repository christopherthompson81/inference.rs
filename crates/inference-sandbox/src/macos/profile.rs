...
mod tests {
    ...
    fn workdir_gets_read_write_rules() {
        let profile = render(&SandboxPolicy {
            session_workdir: Some(PathBuf::from("/tmp/inference workdir")),
            ..SandboxPolicy::default()
        });

        assert!(profile.contains("(allow file-read* (literal \"/tmp/inference workdir\"))"));
        assert!(profile.contains("(allow file-write* (literal \"/tmp/inference workdir\"))"));
        assert!(profile.contains("(allow file-write* (subpath \"/tmp/inference workdir\"))"));
        assert!(
            !profile.contains("(allow file-map-executable (literal \"/tmp/inference workdir\"))")
        );
        assert!(
            !profile.contains("(allow file-map-executable (subpath \"/tmp/inference workdir\"))")
        );
    }
