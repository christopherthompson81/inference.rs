...
fn flash_attn_acausal() -> Result<()> {
    ...
    let ys2 = {
        ...
        inference_flash_attn::flash_attn(&q, &k, &v, 0.5, false)?.transpose(1, 2)?
    };
...
fn flash_attn_varlen_paged_mm_prefix_windowed() -> Result<()> {
    ...
    let ys_causal = inference_flash_attn::flash_attn_varlen_paged_windowed(
        &q,
        &k_paged,
        &v_paged,
        &seqlens,
    ...
    let ys = inference_flash_attn::flash_attn_varlen_paged_windowed(
        &q,
        &k_paged,
        &v_paged,
        &seqlens,
...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        inference_flash_attn::flash_attn_varlen(
