...
pub struct ShellConfig {
    ...
    #[serde(default)]
    pub permission: inference_mcp::AgentPermission,
}
...
impl Default for ShellConfig {
    fn default() -> Self {
        Self {
            ...
            sandbox_policy: None,
            permission: inference_mcp::AgentPermission::Auto,
        }
    }
}
...
impl ShellManager {
    ...
    pub fn get_tool_callbacks(&self) -> ToolCallbacksWithTools {
        ...
        let callback: Arc<inference_mcp::MultimodalToolCallback> = Arc::new(
        ...
        let sessions = Arc::clone(&self.sessions);
        let ctx = self.spawn_ctx();
        let surface_outputs_callback: Arc<inference_mcp::MultimodalToolCallback> = Arc::new(
            move |func: &CalledFunction, tc: &inference_mcp::ToolCallContext| {
                let sessions = Arc::clone(&sessions);
                let ctx = ctx.clone();
                let session_id = tc
...
fn mount_input_files(
    ...
    files: &[inference_mcp::ToolInputFile],
) -> anyhow::Result<()> {
    let key = files
        .iter()
