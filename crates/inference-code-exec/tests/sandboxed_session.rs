...
use inference_sandbox::{NetworkMode, SandboxPolicy};
...
fn exec_json(
    callbacks: &std::collections::HashMap<String, inference_mcp::ToolCallbackWithTool>,
    session_id: &str,
    code: &str,
) -> String {
...
fn exec_json_with_permission(
    ...
    let exec = callbacks
        .get(inference_code_exec::EXECUTE_PYTHON_TOOL_NAME)
        .expect("execute_python tool registered");

    let func = CalledFunction {
        name: inference_code_exec::EXECUTE_PYTHON_TOOL_NAME.to_string(),
    ...
    match result {
        inference_mcp::ToolOutput::Multimodal { text, .. } => text,
        inference_mcp::ToolOutput::Text(t) => t,
    }
}
...
fn reset_json(
    ...
    let reset = callbacks
        .get(inference_code_exec::RESET_SESSION_TOOL_NAME)
        .expect("reset_python_session tool registered");

    let func = CalledFunction {
        name: inference_code_exec::RESET_SESSION_TOOL_NAME.to_string(),
        arguments: "{}".to_string(),
    };
    let ctx = ToolCallContext {
        session_id: Some(session_id.to_string()),
