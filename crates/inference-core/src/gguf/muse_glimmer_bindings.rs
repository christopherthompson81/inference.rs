...
mod tests {
    ...
    fn local_archives_have_complete_native_bindings() -> Result<()> {
        let path = std::env::var("INFERENCE_RS_MUSE_GLIMMER_GGUF")
            .map_err(|_| anyhow::anyhow!("INFERENCE_RS_MUSE_GLIMMER_GGUF is not set"))?;
        let mmproj_path = std::env::var("INFERENCE_RS_MUSE_GLIMMER_MMPROJ_GGUF")
            .map_err(|_| anyhow::anyhow!("INFERENCE_RS_MUSE_GLIMMER_MMPROJ_GGUF is not set"))?;
