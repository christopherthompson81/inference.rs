...
impl<'a, R: std::io::Seek + std::io::Read> Content<'a, R> {
    ...
    /// This will also log tensor name, shape and dtype to `inference_gguf_tensors.txt` is DEBUG is enabled.
    pub fn print_metadata(&self) -> anyhow::Result<()> {
        ...
        if DEBUG.load(std::sync::atomic::Ordering::Relaxed) {
            fs::write(
                "inference_gguf_tensors.txt",
                serde_json::to_string_pretty(&tensors).expect("Serialization failed."),
            )?;

            info!("Debug is enabled, wrote the names and information about each tensor to `inference_gguf_tensors.txt`.");
        }
