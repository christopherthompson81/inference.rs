...
impl DynamicLoraRuntime {
    ...
    fn install_inner(
        ...
        let plan = inference_quant::plan_dynamic_lora_weights(&self.layers, config, weights)
        ...
        let weights = Arc::new(
            inference_quant::LoraAdapterWeights::new(self.layers.runtime_id(), loaded)
                .map_err(LoraAdapterError::Load)?,
        );
...
fn adapter_bytes(weights: &inference_quant::DynamicLoraWeights) -> Result<u64, LoraAdapterError> {
    fn add_tensor(
        total: u64,
        tensor: &Tensor,
...
mod tests {
    ...
    fn expert_registry() -> (
        ...
        Arc<inference_quant::LoraExpertSiteHandle>,
    ) {
    ...
    fn expert_directory_load_runs_through_runtime_execution() {
        ...
        let output = with_lora_execution(Some(execution), || {
            let lora = inference_quant::LoraExpertExecution::current(&site)
                .unwrap()
