...
const PAGED_RECURRENT_PREFIX_CHECKPOINTS_USED_METRIC: &str =
    "inference_paged_recurrent_prefix_checkpoints_used";
const PAGED_RECURRENT_PREFIX_BYTES_USED_METRIC: &str =
    "inference_paged_recurrent_prefix_bytes_used";
const PAGED_RECURRENT_PREFIX_OWNER_EVICTIONS_METRIC: &str =
    "inference_paged_recurrent_prefix_owner_evictions_total";
...
impl PrefixCacheManagerV2 {
    ...
    pub fn evict_caches(&mut self) -> Result<usize> {
        ...
        if n_evicted > 0 {
            metrics::counter!("inference_prefix_cache_evictions_total").increment(n_evicted as u64);
        }
        Ok(n_evicted)
    }
    ...
    pub fn evict_all_caches(&mut self) -> Result<usize> {
        ...
        self.publish_paged_recurrent_metrics();
        if len > 0 {
            metrics::counter!("inference_prefix_cache_evictions_total").increment(len as u64);
        }
        Ok(len)
    }
