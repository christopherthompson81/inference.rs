...
impl Gemma4Router {
    ...
    fn forward(&self, xs: &Tensor, per_expert_scale: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let logits = match &self.proj_lora {
            Some(site) => inference_quant::apply_dynamic_lora_delta(site, &router_input, logits)?,
            None => logits,
        };
...
impl Attention {
    ...
    fn new(
        ...
        let (q_proj, k_proj, v_proj, merged_qkv_proj, k_norm, v_norm_rms) = if is_shared {
            ...
            let kv_shard = inference_quant::compute_kv_shard(num_kv_heads, head_dim, comm)?;
            let is_k_eq_v = !sliding && cfg.attention_k_eq_v;
            let q_shard = inference_quant::Shard::Simple {
                dim: 0,
                rank: comm.rank(),
                world_size: comm.world_size(),
            };
            let kv_dim = num_kv_heads * head_dim;
            let (names, dims, shards): (&[&str], &[usize], &[inference_quant::Shard]) = if is_k_eq_v
            {
                (
                    &["q_proj", "k_proj"],
...
impl DecoderLayer {
    ...
    fn new(
        ...
        {
            let gate = inference_quant::linear_no_bias(
                cfg.hidden_size,
                ple_dim,
                &None,
                mapper.set_device(layer_idx, vb.pp("per_layer_input_gate"), loading_isq),
            )?;
            let proj = inference_quant::linear_no_bias(
                ple_dim,
                cfg.hidden_size,
                &None,
...
impl TextModel {
    pub fn new(
        ...
        let (embed_tokens_per_layer, per_layer_model_projection, per_layer_projection_norm) =
            if ple_dim > 0 {
                ...
                let ple_proj = inference_quant::linear_no_bias(
                    cfg.hidden_size,
                    cfg.num_hidden_layers * ple_dim,
                    &None,
                    mapper.set_nm_device(
    ...
    fn kv_sharing_fast_prefill_plan(
        ...
        if requires_full_prefill_queries
            ...
            || inference_quant::has_active_lora_execution()
        {
            return Ok(None);
        }
