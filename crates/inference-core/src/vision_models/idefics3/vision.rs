...
impl Attention {
    fn new(config: Idefics3VisionConfig, vb: ShardedVarBuilder) -> Result<Self> {
        ...
        let q_proj = inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("q_proj"))?;
        let k_proj = inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("k_proj"))?;
        let v_proj = inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("v_proj"))?;
        let o_proj = inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("out_proj"))?;
...
impl VisionMLP {
    fn new(config: Idefics3VisionConfig, vb: ShardedVarBuilder) -> Result<Self> {
        let fc1 = inference_quant::linear(
            config.hidden_size,
            config.intermediate_size,
            &None,
            vb.pp("fc1"),
        )?;
        let fc2 = inference_quant::linear(
            config.intermediate_size,
            config.hidden_size,
            &None,
            vb.pp("fc2"),
