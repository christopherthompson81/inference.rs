...
impl Attention {
    fn new(config: &VisionConfig, vb: ShardedVarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            ...
            q_proj: inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("q_proj"))?,
            k_proj: inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("k_proj"))?,
            v_proj: inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("v_proj"))?,
            o_proj: inference_quant::linear(embed_dim, embed_dim, &None, vb.pp("out_proj"))?,
        })
    }
...
impl Mlp {
    fn new(config: &VisionConfig, vb: ShardedVarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            fc1: inference_quant::linear(
                config.hidden_size,
                config.intermediate_size,
                &None,
                vb.pp("fc1"),
            )?,
            fc2: inference_quant::linear(
                config.intermediate_size,
                config.hidden_size,
                &None,
                vb.pp("fc2"),
