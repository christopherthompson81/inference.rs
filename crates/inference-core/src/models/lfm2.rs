...
impl Mlp {
    fn new(
        ...
        let w1 = inference_quant::linear_no_bias(
            cfg.hidden_size,
            intermediate_size,
            &cfg.quantization_config,
            vb.pp("w1"),
        )?;
        let w2 = inference_quant::linear_no_bias(
            intermediate_size,
            cfg.hidden_size,
            &cfg.quantization_config,
            vb.pp("w2"),
        )?;
        let w3 = inference_quant::linear_no_bias(
            cfg.hidden_size,
            intermediate_size,
            &cfg.quantization_config,
            vb.pp("w3"),
...
impl Attention {
    ...
    fn new(
        ...
        let kv_shard = inference_quant::compute_kv_shard(cfg.num_key_value_heads, head_dim, comm)?;
...
impl ShortConv {
    fn new(
        ...
        let in_proj = inference_quant::linear_b(
            cfg.hidden_size,
            3 * cfg.hidden_size,
            cfg.conv_bias,
            &cfg.quantization_config,
        ...
        let out_proj = inference_quant::linear_b(
            cfg.hidden_size,
            cfg.hidden_size,
            cfg.conv_bias,
            &cfg.quantization_config,
