...
fn local_resolution_hint(model_id: &Path) -> String {
    if looks_like_local_path(model_id) {
        ...
        format!(
            "No local directory exists at `{}`, so inference.rs treated it as a Hugging Face model ID.",
            model_id.display()
        )
    }
}
...
fn hf_error(
    ...
    let mut message = match status_code {
        Some(code @ (401 | 403)) => format!(
            "Could not access {target} on Hugging Face (HTTP {code}). Run `inference login` or set `HF_TOKEN` if this is a private or gated repo."
        ),
...
mod tests {
    ...
    fn explicit_cache_path_is_preserved() {
        let path = std::env::temp_dir().join("inference-explicit-hf-cache");
