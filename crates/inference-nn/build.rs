...
fn cuda_build_dir(out_dir: &std::path::Path, component: &str) -> std::path::PathBuf {
    ...
    let build_dir = std::path::PathBuf::from(root).join("nn").join(component);
...
fn main() {
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        let header_hash_arg = format!(
            "-DINFERENCE_RS_NN_CUDA_HEADER_HASH=0x{:016x}",
        ...
        let out_file = if target.contains("msvc") {
            // Windows case
            out_dir.join("inferencecuda.lib")
        } else {
            out_dir.join("libinferencecuda.a")
        };
...
fn set_cuda_toolkit_version() -> Option<u32> {
    ...
    {
        println!("cargo:rustc-env=INFERENCE_RS_BUILD_CUDA_VERSION={version}");
        println!("cargo:rustc-env=INFERENCE_RS_BUILD_CUDA_VERSION_CODE={code}");
    }
    Some(code)
}
