...
const RETAINED_PREFIX_ENTRIES_METRIC: &str = "inference_paged_prefix_retention_entries";
const RETAINED_PREFIX_HASHES_METRIC: &str = "inference_paged_prefix_retention_hashes";
const RETAINED_PREFIX_PRESSURE_EVICTIONS_METRIC: &str =
    "inference_paged_prefix_retention_pressure_evictions_total";
const RETAINED_PREFIX_PHYSICAL_BLOCKS_METRIC: &str = "inference_kv_cache_blocks_prefix_retained";
const RETAINED_PREFIX_FREE_BLOCKS_METRIC: &str = "inference_paged_prefix_retention_free_blocks";
const ALLOCATION_PRESSURE_REASON: &str = "allocation_pressure";
const RESET_REASON: &str = "reset";

#[derive(Clone)]
pub struct PrefixBlockRetention {
...
pub struct PrefixBlockRetentionRevocationMonitor {
...
pub struct PrefixBlockRetentionLease {
    id: u64,
    active: Arc<AtomicBool>,
...
impl PrefixBlockRetention {
    ...
    pub fn revocation_monitor(&self) -> PrefixBlockRetentionRevocationMonitor {
    ...
    pub fn retain(&self, hashes: &[BlockHash]) -> PrefixBlockRetentionLease {
        assert!(
...
impl BlockPool {
    ...
    pub fn prefix_block_retention(&self) -> PrefixBlockRetention {
        self.prefix_block_retention.clone()
    }

    pub fn set_retention_group_ids(&mut self, group_ids: Vec<u32>) {
        assert!(
    ...
    fn maybe_evict_cached_block(&mut self, block_id: usize) {
        ...
        if !block_hashes.is_empty() {
            metrics::counter!("inference_prefix_cache_evictions_total")
    ...
    pub fn reset_prefix_cache(&mut self) -> bool {
        ...
        if evicted > 0 {
            metrics::counter!("inference_prefix_cache_evictions_total").increment(evicted as u64);
        }
