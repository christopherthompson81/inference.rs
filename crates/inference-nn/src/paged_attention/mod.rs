...
pub fn device_memory_cap(available_bytes: usize, device: &Device) -> usize {
    if device.is_cpu() {
        available_bytes
    } else {
...
pub fn block_aligned_sliding_window_start(
    full_len: usize,
    query_len: usize,
    window: usize,
    block_size: usize,
...
pub struct PagedAttentionConfig {
    ...
    pub mem_gpu: MemoryGpuConfig,
    pub cache_type: PagedCacheType,
    pub serving_capacity: Option<usize>,
    pub base_device_memory_reservation_bytes: usize,
    pub primary_activation_memory_reservation_bytes: usize,
...
impl PagedAttentionConfig {
    ...
    pub fn memory_reservations(&self) -> anyhow::Result<CacheMemoryReservations> {
...
pub fn calculate_cache_config(
    ...
    for device in cache_devices {
        ...
        let affine_bytes = if post_load_memory.is_some() {
            inference_quant::gguf_affine_budget_bytes(device, model_dtype)
        } else {
            0
        };
