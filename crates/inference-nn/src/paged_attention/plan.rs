...
impl PrefixPrefillPlan {
    pub fn choose(input: PrefixPrefillPlanInput) -> Self {
        ...
        #[cfg(all(feature = "cuda", target_family = "unix"))]
        if inference_paged_attn::USE_FA3_FP8_PAGED && fa3_paged_prefill_supported(input) {
...
#[cfg(all(feature = "cuda", target_family = "unix"))]
pub fn fa3_paged_prefill_supported(input: PrefixPrefillPlanInput) -> bool {
    input.fa3_supported
        && input.device_is_cuda
        ...
        && input.query_layout_is_dense
        && input.query_len > 0
        && input.query_len <= inference_paged_attn::FA3_DECODE_MAX_QUERY_LEN
...
#[derive(Clone, Copy, Debug)]
pub struct GatherPrefillWorkspaceRequest<'a> {
    pub query_lens: &'a [usize],
    pub kv_lens: &'a [usize],
    pub q_heads: usize,
    pub kv_heads: usize,
...
pub fn gather_prefill_workspace_for_lengths(
...
mod tests {
    ...
    fn prompt_workspace_tracks_fa3_support_per_layer() {
        ...
        let expected_bytes = if inference_paged_attn::USE_FA3_FP8_PAGED {
            let fa3 =
