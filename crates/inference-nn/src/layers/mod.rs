...
pub fn yarn_inv_freq_and_attention_factor(
    cfg: &YarnRopeConfig,
    device: &Device,
) -> Result<(Tensor, f32)> {
...
pub fn apply_rotary_preselected_qk(
    q: &Tensor,
    k: &Tensor,
    cos: &Tensor,
    sin: &Tensor,
    ...
    let (q, k) = inference_quant::rotary::apply_rotary_qk_preselected(q, k, cos, sin, is_gpt_neox)?;
...
pub fn apply_rotary_qk(
    ...
    let (q, k) = inference_quant::rotary::apply_rotary_qk(q, k, cos, sin, positions, is_gpt_neox)?;
    Ok((post_rope_output(q)?, post_rope_output(k)?))
}
...
pub fn apply_rotary_q(
    ...
    post_rope_output(inference_quant::rotary::apply_rotary_q(
        q,
        cos,
        sin,
        positions,
...
impl Mlp {
    ...
    pub fn forward_with_add_rms_norm(
        ...
        if let Some(layout @ ActivationScaleLayout::GroupMajor { .. }) = layout {
            ...
            let fused = inference_quant::fused_add_rms_norm_quantized(
                branch,
                residual,
                norm.weight(),
                norm.eps() as f32,
