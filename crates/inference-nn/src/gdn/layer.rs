...
pub struct GdnForwardContext<'a> {
...
enum GdnCoreOutput {
    ...
    Normalized(Tensor),
    #[cfg(feature = "cuda")]
    Quantized(inference_quant::QuantizedActivation),
}
...
pub fn speculative_checkpoint_dims_supported(dims: &GdnDims) -> bool {
...
pub fn speculative_state_commit_dims_supported(dims: &GdnDims) -> bool {
    let Some(key_dim) = dims.num_k_heads.checked_mul(dims.head_k_dim) else {
        return false;
    };
    let Some(value_dim) = dims.num_v_heads.checked_mul(dims.head_v_dim) else {
...
impl GatedDeltaNet {
    ...
    pub fn speculative_checkpoints_supported(
        &self,
    ...
    pub fn speculative_transitions_supported(
        &self,
        pool: &RecurrentStatePool,
        activation_dtype: DType,
    ) -> bool {
    ...
    pub fn deferred_decode_supported(
        &self,
        pool: &RecurrentStatePool,
        activation_dtype: DType,
    ) -> bool {
    ...
    pub fn transition_commit_config(&self, pool: &RecurrentStatePool) -> GdnTransitionCommitConfig {
    ...
    pub fn pending_transition_spec(&self, max_rows: usize) -> GdnPendingTransitionSpec {
    ...
    pub fn deferred_state_spec(&self) -> GdnDeferredStateSpec {
    ...
    pub fn speculative_state_commit_supported(
        &self,
    ...
    pub fn input_activation_quantization_scheme_for(
        &self,
        x: &Tensor,
    ...
    pub fn preferred_input_activation_scale_layout_for(
        &self,
        x: &Tensor,
    ...
    pub fn forward_quantized_with_context(
        &self,
        x: &Tensor,
    ...
    pub fn forward_projected_prefill_core(
        &self,
        projected: &GdnCoreProjection,
        cache: &mut GdnLayerCache,
    ) -> Result<Tensor> {
    ...
    pub fn forward_projected_padded_prefill_core(
        &self,
        projected: &GdnCoreProjection,
        cache: &mut GdnLayerCache,
        layout: &PackedGdnLayout,
    ...
    pub fn project(&self, x: &Tensor, batch_size: usize, seq_len: usize) -> Result<GdnProjection> {
    ...
    pub fn finish_projected_recurrent(&self, output: Tensor, z: Tensor) -> Result<Tensor> {
