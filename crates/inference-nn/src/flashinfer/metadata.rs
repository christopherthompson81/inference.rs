...
pub fn decode_split_capacity_pages(block_size: usize) -> usize {
...
pub fn make_paged_kv_tensors<T: BlockTableRows + ?Sized>(
    tables: &T,
    context_lens: &[usize],
    block_size: usize,
...
pub fn make_paged_kv_decode_tensors<T: BlockTableRows + ?Sized>(
    tables: &T,
    context_lens: &[usize],
    block_size: usize,
...
pub fn make_paged_kv_decode_tensors_from_lens(
    context_lens: &[usize],
    block_size: usize,
    split_pages: Option<usize>,
) -> Result<(Tensor, Tensor, Tensor, Tensor, Tensor)> {
...
#[cfg(all(feature = "cuda", target_family = "unix"))]
pub fn fa3_prefill_cache_num_sm(
    key_cache: &Tensor,
    value_cache: &Tensor,
    q_heads: usize,
    kv_heads: usize,
...
#[cfg(all(feature = "cuda", target_family = "unix"))]
pub struct Fa3PrefillWorkspaceRegistration {
...
#[cfg(all(feature = "cuda", target_family = "unix"))]
pub fn with_fa3_prefill_workspace<R>(
    metadata: &FlashInferMetadata,
    key: Fa3DecodeScheduleKey,
    key_cache: &Tensor,
    execution_device: &Device,
    ...
    let result = (|| {
        inference_paged_attn::fa3_prepare_paged_metadata(
            ...
            inference_paged_attn::Fa3PagedMetadataLayout::per_sequence(),
        )?;
        run(&buffers)
    })();
