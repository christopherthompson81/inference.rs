...
impl ExpertProj {
    ...
    pub fn split_canonical_key(key: &str) -> Option<(&str, Self)> {
    ...
    pub fn name_in(self, names: &ExpertProjNames) -> &'static str {
        match self {
...
impl BackendChoice {
    pub(super) fn new(
        ...
        Self {
            ...
            immediate_isq: inference_quant::get_immediate_isq().is_some(),
            act,
        }
    }
}
...
#[cfg(feature = "cuda")]
pub(super) fn gated_act(act: Activation) -> Result<inference_quant::moe::cuda::GatedAct> {
    match act {
        Activation::Silu => Ok(inference_quant::moe::cuda::GatedAct::Silu),
...
impl MoEExpertsBackend {
    pub(super) fn from_env() -> Option<Self> {
        let force = std::env::var("INFERENCE_RS_MOE_BACKEND").ok()?;
...
fn cutile_arch_supported(device: &Device) -> bool {
    match device {
        Device::Cuda(dev) => inference_quant::cutile::device_supported(dev),
        _ => false,
    }
}
...
fn cutile_jit_available(device: &Device) -> bool {
    match device {
        Device::Cuda(dev) => inference_quant::cutile::jit_available(dev),
        _ => false,
    }
}
...
fn cutlass_moe_supported(device: &Device) -> bool {
    match device {
        Device::Cuda(dev) => inference_quant::moe::cutlass_moe_available(dev),
        _ => false,
    }
}
...
mod tests {
    ...
    fn grouped_activation_mapping_is_exhaustive() {
        assert_eq!(
            ...
            inference_quant::moe::cuda::GatedAct::Silu
        );
        assert_eq!(
            gated_act(Activation::NewGelu).unwrap(),
            inference_quant::moe::cuda::GatedAct::GeluTanh
        );
        assert_eq!(
