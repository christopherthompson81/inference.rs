...
impl ExpertSourceLayout {
    ...
    pub(super) fn read_stack(
        &self,
        source: &inference_quant::safetensors::MmapedSafetensors,
        prefix: &str,
        proj: ExpertProj,
    ) -> Result<Tensor> {
        match self {
...
pub fn rebuild_expert_projection(
    source: &inference_quant::safetensors::MmapedSafetensors,
    shapes: &std::collections::HashMap<String, Vec<usize>>,
    key: &str,
...
mod tests {
    ...
    fn open_source(
        ...
        inference_quant::safetensors::MmapedSafetensors,
        HashMap<String, Vec<usize>>,
    ) {
        let source = unsafe { inference_quant::safetensors::MmapedSafetensors::new(path).unwrap() };
        let shapes = source
            .tensors()
            .into_iter()
