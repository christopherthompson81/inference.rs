...
pub fn sinks_backend_is_available(query: &Tensor, head_size: usize) -> bool {
...
fn sinks_attn_regular(
    ...
    if q.device().is_metal() {
        return inference_quant::flash_attn_sinks_metal(
            q,
            k,
            v,
            Some(sinks),
...
fn sinks_attn_varlen(
    ...
    #[cfg(feature = "metal")]
    if device.is_metal() {
        return inference_quant::flash_attn_sinks_varlen_metal(
            q,
            &k_packed,
            &v_packed,
            Some(sinks),
