...
pub struct FlashParams {
    ...
    pub varlen_segment_lens: Option<Vec<usize>>,
}
...
pub fn flash_param_devices(device: &Device, mapper: Option<&dyn DeviceMapper>) -> Vec<Device> {
    mapper
        .map(|mapper| mapper.get_unique_devices())
        .unwrap_or_else(|| vec![device.clone()])
}

pub fn cumulative_seqlens_map(
    lengths: &[u32],
    devices: &[Device],
...
pub fn packed_rope_positions(seqlen_offsets: &[usize], query_lens: &[usize]) -> Result<Vec<u32>> {
