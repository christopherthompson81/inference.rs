...
pub const SPARSE_REJECTION_STATUS_OK: u32 = 0;
pub const SPARSE_REJECTION_STATUS_NEEDS_CPU: u32 = 1;
pub const SPARSE_REJECTION_STATUS_INVALID_Q: u32 = 2;
pub const SPARSE_REJECTION_STATUS_INVALID_TARGET: u32 = 3;
pub const SPARSE_REJECTION_STATUS_INVALID_RNG: u32 = 4;
pub const SPARSE_REJECTION_INVALID_VALUE: u32 = u32::MAX;
pub const SPARSE_REJECTION_OUTCOME_WIDTH: usize = 3;
pub const SPARSE_REJECTION_MAX_Q_WIDTH: usize = 128;

const OP: &str = "sparse_rejection_cuda";

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SparseRejectionMode {
    Categorical,
    BoundedTopK { max_top_k: usize },
}

#[cfg(test)]
pub struct SparseRejectionInput<'a> {
    pub target_logits: &'a Tensor,
    pub draft_tokens: &'a Tensor,
    pub q_token_ids: &'a Tensor,
    pub q_probs: &'a Tensor,
    pub inverse_temperatures: &'a Tensor,
    pub target_top_k: &'a Tensor,
    pub top_p: &'a Tensor,
    pub min_p: &'a Tensor,
    pub accept_uniforms: &'a Tensor,
    pub sample_uniforms: &'a Tensor,
    pub mode: SparseRejectionMode,
}

pub struct SparseRejectionWorkspaceInput<'a> {
    pub target_logits: &'a Tensor,
    pub proposal: SparseRejectionProposalInput<'a>,
    pub draft_tokens: SparseRejectionDraftInput<'a>,
    pub inverse_temperatures: &'a [f32],
    pub target_top_k: &'a [u32],
    pub top_p: &'a [f32],
    pub min_p: &'a [f32],
    pub accept_uniforms: &'a [f32],
    pub sample_uniforms: &'a [f32],
    pub mode: SparseRejectionMode,
}

#[derive(Clone, Copy)]
pub enum SparseRejectionDraftInput<'a> {
    Host(&'a [u32]),
    #[cfg(test)]
    Device(&'a Tensor),
    DeviceRows(&'a [Tensor]),
...
#[derive(Clone, Copy)]
pub enum SparseRejectionProposalInput<'a> {
    Deterministic,
    #[cfg(test)]
    Sparse {
        token_ids: &'a Tensor,
...
pub struct CudaSparseRejectionSubmission {
    workspace_id: u64,
    generation: u64,
    batch: usize,
    drafts: usize,
...
#[derive(Debug)]
pub struct SparseRejectionCompletion {
    pub rows: Vec<SparseRejectionRow>,
    pub draft_tokens: Vec<Vec<u32>>,
}

pub struct SparseRejectionOutput {
    _outcomes: Tensor,
    _inputs: Vec<Tensor>,
...
impl SparseRejectionOutput {
    #[cfg(test)]
    pub fn to_rows(&self) -> Result<Vec<SparseRejectionRow>> {
        self._outcomes
            .to_vec2::<u32>()?
            .into_iter()
            .map(|row| {
...
pub fn sparse_rejection_cuda_submit(
...
pub fn sparse_rejection_cuda_complete(
    cache: &mut Option<CudaSparseRejectionWorkspace>,
...
pub fn sparse_rejection_cuda(input: SparseRejectionInput<'_>) -> Result<SparseRejectionOutput> {
