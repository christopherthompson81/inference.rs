...
pub const GDN_SPEC_COMMIT_MAX_K: usize = 256;
pub const GDN_SPEC_CHECKPOINT_MAX_CONV_WIDTH: usize = 16;
pub const GDN_SPEC_CHECKPOINT_MAX_K: usize = 256;
pub const GDN_SPEC_FUSED_MAX_TOKENS: usize = 8;
pub const GDN_CHANNEL_BLOCK_SIZE: usize = 256;
...
#[cfg(feature = "cuda")]
const GDN_DECODE_KERNEL_ENV: &str = "INFERENCE_RS_GDN_DECODE_KERNEL";
#[cfg(feature = "cuda")]
const GDN_PREFILL_KERNEL_ENV: &str = "INFERENCE_RS_GDN_PREFILL_KERNEL";
#[cfg(feature = "cuda")]
...
pub fn recurrent_state_dtype_supported(dtype: DType) -> bool {
    matches!(dtype, DType::F16 | DType::BF16 | DType::F32)
}

pub fn deferred_decode_batch_supported(batch_size: usize) -> bool {
...
#[derive(Clone, Copy)]
#[cfg_attr(not(feature = "cuda"), allow(dead_code))]
pub struct GdnPackedToPadded<'a> {
    pub source: &'a Tensor,
...
#[derive(Clone, Copy)]
#[cfg_attr(not(feature = "cuda"), allow(dead_code))]
pub struct GdnPaddedToPacked<'a> {
    pub source: &'a Tensor,
    pub cu_seqlens: &'a Tensor,
    pub batch_size: usize,
    pub token_count: usize,
...
#[derive(Clone, Copy)]
#[cfg_attr(not(feature = "cuda"), allow(dead_code))]
pub struct GdnRaggedConvState<'a> {
...
#[cfg(not(feature = "cuda"))]
pub fn try_gdn_packed_to_padded_cuda(_context: GdnPackedToPadded<'_>) -> Result<Option<Tensor>> {
    Ok(None)
}
...
#[cfg(not(feature = "cuda"))]
pub fn try_gdn_padded_to_packed_cuda(_context: GdnPaddedToPacked<'_>) -> Result<Option<Tensor>> {
    Ok(None)
}
...
impl candle_core::InplaceOp1 for FlashInferGdnSm90Launch<'_> {
    ...
    fn cuda_fwd(
        ...
        let status = unsafe {
            crate::cuda::ffi::inference_flashinfer_gdn_sm90_launch(
...
fn cutile_prefill_supported(launch: &FusedPrefillRecurrence<'_>) -> Result<bool> {
    ...
    Ok(inference_quant::cutile::gdn_prefill_supported(
        dev,
        launch.head_k_dim,
        launch.head_v_dim,
    ))
...
fn cutile_prefill(launch: FusedPrefillRecurrence<'_>) -> Result<FusedPrefillOutput> {
    ...
    let token_stride = seq_len.div_ceil(inference_quant::cutile::GDN_PREFILL_CHUNK)
        * inference_quant::cutile::GDN_PREFILL_CHUNK;
    ...
    let output = inference_quant::cutile::cutile_gdn_prefill(
        &inference_quant::cutile::GdnPrefillArgs {
            q: &q,
            k: &k,
            v: &v,
            g: &g,
...
#[cfg(feature = "cuda")]
pub fn rmsnorm_gated_quantized_cuda(
    x: &Tensor,
    gate: &Tensor,
    weight: &Tensor,
    eps: f64,
