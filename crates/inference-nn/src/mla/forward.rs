...
#[cfg(all(feature = "cuda", target_family = "unix"))]
const INFERENCE_RS_NO_MLA: &str = "INFERENCE_RS_NO_MLA";
...
pub fn mla_decode_forward(
    ...
    inference_paged_attn::concat_and_cache_mla(
        &ckv_flat,
        &k_pe_flat,
        key_cache,
        value_cache,
    ...
    let attn_latent = inference_paged_attn::flashinfer_mla_decode(
        &ql_nope,
        &q_pe,
        key_cache,
        value_cache,
...
pub fn mla_cache_forward(
    ...
    if !needs_prefix && !matches!(attention_mask, AttentionMask::None) {
        ...
        if total_prefix_tokens > 0 {
            ...
            let (ckv_prefix, kpe_prefix) = inference_paged_attn::gather_mla_cache(
                key_cache,
                value_cache,
                &block_tables_seq,
                &cu_seq_lens,
