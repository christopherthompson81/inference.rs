...
/// Python wrapper for [`inference_core::RequestedFile`].
#[pyclass]
#[derive(Clone, Debug)]
...
impl From<RequestedFile> for inference_core::RequestedFile {
    fn from(f: RequestedFile) -> Self {
        inference_core::RequestedFile {
            name: f.name,
            format: f.format,
...
pub struct InputFile {
    pub(crate) file: inference_core::File,
}
...
impl InputFile {
    fn from_bytes(name: String, bytes: Vec<u8>, mime_type: Option<String>) -> Self {
        Self {
            file: inference_core::File::from_bytes(
                inference_core::File::make_upload_id(),
                name,
                mime_type,
                inference_core::FILE_PURPOSE_USER_DATA.to_string(),
                inference_core::FileSource {
...
impl From<InputFile> for inference_core::File {
