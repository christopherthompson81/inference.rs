...
use inference_core::{
    ...
    InferenceRs, InferenceRsBuilder, InferenceRsError, LlguidanceGrammar, Loader,
    LoraAdapterError as CoreLoraAdapterError, LoraAdapterLoadPolicy, LoraAdapterSpec,
    LoraRuntimeConfig, MemoryGpuConfig, MtpConfig, MtpRuntimeConfig, MultimodalLoaderBuilder,
    MultimodalSpecificConfig, NormalLoaderBuilder, NormalRequest, NormalSpecificConfig,
    PagedAttentionConfig, PagedCacheType, Request as _Request, RequestMessage, Response,
    ResponseOk, SamplingParams, SchedulerConfig, SearchEmbeddingModel, SpeculativeConfig,
    SpeechLoader, StopTokens, TokenSource, TokenizationRequest, Tool, Topology, UqffWriteConfig,
};
use inference_core::{
    CalledFunction, SearchCallback, SearchFunctionParameters, SearchResult, ToolCallback,
    ToolCallbacks,
};
use inference_mcp::{McpClientConfig, McpServerConfig, McpServerSource};
...
fn lora_adapter_error_code(error: &InferenceRsError) -> &'static str {
    match error {
        InferenceRsError::LoraAdapter(error) => match error {
        ...
        InferenceRsError::ModelNotFound(_) => "model_not_found",
        InferenceRsError::ModelReloading(_)
        | InferenceRsError::ModelAlreadyLoaded(_)
        | InferenceRsError::ModelAlreadyUnloaded(_) => "model_state_conflict",
        InferenceRsError::NoLoaderConfig(_) => "invalid_model_operation",
        InferenceRsError::SenderPoisoned => "service_unavailable",
        InferenceRsError::EnginePoisoned
        | InferenceRsError::ReloadFailed(_)
        | InferenceRsError::Other(_) => "internal_error",
    }
}
...
fn lora_adapter_core_py_error(error: InferenceRsError) -> PyApiErr {
...
impl From<inference_core::LoraAdapterInfo> for LoraAdapterInfoPy {
    fn from(info: inference_core::LoraAdapterInfo) -> Self {
        Self {
            alias: info.alias,
            source: info.source,
...
impl From<inference_core::LoraResidentGenerationInfo> for LoraResidentGenerationInfoPy {
...
impl From<inference_core::LoraRuntimeStatus> for LoraRuntimeStatusPy {
    fn from(status: inference_core::LoraRuntimeStatus) -> Self {
        Self {
...
fn get_device(seed: Option<u64>) -> &'static Result<Device> {
    DEVICE.get_or_init(|| {
        let device = if inference_core::distributed::use_nccl() {
            Device::Cpu
        } else {
...
fn wrap_tool_callback(cb: PyObject) -> Arc<ToolCallback> {
    Arc::new(
        move |func: &CalledFunction, _ctx: &inference_core::ToolCallContext| {
            Python::with_gil(|py| {
...
impl Runner {
    ...
    fn new(
        ...
        let no_paged_attn = if device.is_cuda() || inference_core::distributed::use_nccl() {
            no_paged_attn
        ...
        let mut builder =
            InferenceRsBuilder::new(pipeline, scheduler_config, false, search_embedding_model);
        if let Some(cb) = cb {
        ...
        if let Some(code_exec) = code_execution_config {
            ...
            #[cfg(not(feature = "code-execution"))]
            {
                ...
                return Err(util::PyApiErr(PyValueError::new_err(
                    "code_execution_config requires the 'code-execution' feature; rebuild inference with `--features code-execution`",
                )));
            }
        }
        if let Some(shell_config) = shell_config {
            ...
            #[cfg(not(feature = "code-execution"))]
            {
                let _ = shell_config;
                return Err(util::PyApiErr(PyValueError::new_err(
                    "shell_config requires the 'code-execution' feature; rebuild inference with `--features code-execution`",
                )));
            }
        }
        let rt = Runtime::new().expect("Failed to create Runner::new runtime");
        let inference = rt.block_on(async {
            builder
    ...
    fn send_chat_completion_request(
        ...
        Python::with_gil(|py| {
            ...
            let tool_choice = request.tool_choice.as_ref().map(|x| match x {
                ToolChoice::Auto => inference_core::ToolChoice::Auto,
                ToolChoice::NoTools => inference_core::ToolChoice::None,
            });
    ...
    fn send_embedding_request(
        ...
        Python::with_gil(|py| {
            ...
            py.allow_threads(move || -> std::result::Result<Vec<Vec<f32>>, String> {
                InferenceRs::maybe_log_request(runner.clone(), debug_repr);
                let sender = runner
    ...
    fn begin_calibration(
        ...
        self.send_calibration(py, inference_core::CalibrationAction::Start, model_id)
    }
    ...
    fn calibration_status(
        ...
        self.send_calibration(py, inference_core::CalibrationAction::Status, model_id)
    }
    ...
    fn apply_calibration(
        ...
        self.send_calibration(
            py,
            inference_core::CalibrationAction::Apply {
    ...
    fn import_session(
        ...
        let session: inference_core::SerializedSession = serde_json::from_str(&session_json)
    ...
    fn find_file(&self, file_id: String) -> Option<inference_core::File> {
        ...
        let policy = match (load_inplace, expected_generation) {
            ...
            (true, Some(generation)) => {
                LoraAdapterLoadPolicy::CompareAndSwap(generation.parse().map_err(
                    |error: inference_core::AdapterGenerationParseError| {
    ...
    fn unload_lora_adapter(
        ...
        let expected_generation = expected_generation
            .map(|generation| {
                generation
                    .parse()
                    .map_err(|error: inference_core::AdapterGenerationParseError| {
                        lora_adapter_py_error("invalid_lora_generation", error.to_string())
                    })
            })
            .transpose()?;
...
impl Runner {
    fn send_calibration(
        ...
        let (tx, mut rx) = channel(1);
        let request = _Request::Calibration(inference_core::CalibrationRequest {
            action,
            response: tx,
        });
        let runner = self.runner.clone();
        py.allow_threads(
            move || -> std::result::Result<anyhow::Result<inference_core::CalibrationStatus>, String> {
                runner
                    .get_sender(model_id.as_deref())
                    .map_err(|e| e.to_string())?
                    .blocking_send(request)
...
impl From<inference_core::CalibrationStatus> for CalibrationStatusPy {
    fn from(s: inference_core::CalibrationStatus) -> Self {
        Self {
...
#[pymodule]
fn inference_rs(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    initialize_logging();
    ...
    m.add_class::<inference_core::File>()?;
    m.add_class::<inference_core::FileSource>()?;
    m.add_class::<ToolChoice>()?;
    m.add_class::<SpeechGenerationResponse>()?;
    m.add_class::<SpeechLoaderType>()?;

    m.add_class::<inference_core::ResponseMessage>()?;
    m.add_class::<inference_core::Delta>()?;
    m.add_class::<inference_core::ResponseLogprob>()?;
    m.add_class::<inference_core::Logprobs>()?;
    m.add_class::<inference_core::Choice>()?;
    m.add_class::<inference_core::ChunkChoice>()?;
    m.add_class::<inference_core::Usage>()?;
    m.add_class::<inference_core::AgenticToolCallRecord>()?;
    m.add_class::<inference_core::ChatCompletionResponse>()?;
    m.add_class::<inference_core::ChatCompletionChunkResponse>()?;
    m.add_class::<inference_core::CompletionChoice>()?;
    m.add_class::<inference_core::CompletionResponse>()?;
    m.add_class::<inference_core::TopLogprob>()?;
    m.add_class::<inference_core::ModelDType>()?;
    m.add_class::<inference_core::ImageGenerationResponseFormat>()?;
    m.add_class::<McpServerSourcePy>()?;
    m.add_class::<McpServerConfigPy>()?;
    m.add_class::<McpClientConfigPy>()?;
    m.add_class::<inference_core::WebSearchOptions>()?;
    m.add_class::<inference_core::SearchContextSize>()?;
    m.add_class::<inference_core::WebSearchUserLocation>()?;
    m.add_class::<inference_core::ApproximateUserLocation>()?;
    Ok(())
}
...
mod mtp_reservation_tests {
    ...
    use inference_core::reserve_external_mtp_memory;
...
mod lora_adapter_error_tests {
    ...
    fn gguf_which(adapters: Option<Vec<LoraAdapter>>, max_rank: usize) -> Which {
        Which::GGUF {
            ...
            calibration_file: None,
            max_edge: None,
            dtype: inference_core::ModelDType::Auto,
            auto_map_params: None,
            multimodal_auto_map_params: None,
            adapters,
            max_adapters: inference_core::DEFAULT_LORA_MAX_ADAPTERS,
            max_rank,
            max_bytes: inference_core::DEFAULT_LORA_MAX_BYTES,
            hf_cache_path: None,
            matformer_config_path: None,
            matformer_slice_name: None,
    ...
    fn lifecycle_errors_have_stable_machine_readable_codes() {
        assert_eq!(
            lora_adapter_error_code(&InferenceRsError::ModelNotFound("model".to_string())),
            "model_not_found"
        );
        assert_eq!(
            lora_adapter_error_code(&InferenceRsError::LoraAdapter(
                CoreLoraAdapterError::RuntimeUnavailable {
        ...
        assert_eq!(
            lora_adapter_error_code(&InferenceRsError::LoraAdapter(CoreLoraAdapterError::Io {
        ...
        assert_eq!(
            lora_adapter_error_code(&InferenceRsError::SenderPoisoned),
            "service_unavailable"
        );
    }
    ...
    fn gguf_dynamic_lora_detection_includes_empty_runtimes_and_custom_limits() {
        assert!(!which_uses_dynamic_lora(&gguf_which(
            None,
            inference_core::DEFAULT_LORA_MAX_RANK,
        )));
        assert!(which_uses_dynamic_lora(&gguf_which(
            Some(Vec::new()),
            inference_core::DEFAULT_LORA_MAX_RANK,
        ...
        assert!(which_uses_dynamic_lora(&gguf_which(
            None,
            inference_core::DEFAULT_LORA_MAX_RANK / 2,
        )));
    }

    #[test]
    fn gguf_dynamic_lora_accepts_multimodal_device_mapping() {
        let mut which = gguf_which(Some(Vec::new()), inference_core::DEFAULT_LORA_MAX_RANK);
        let Which::GGUF {
            mmproj_filename,
            multimodal_auto_map_params,
            ..
    ...
    fn encoder_cache_memory_requires_nonzero_capacity() {
        let mut which = gguf_which(None, inference_core::DEFAULT_LORA_MAX_RANK);
        let Which::GGUF {
            encoder_cache_memory_bytes,
            ..
        } = &mut which
