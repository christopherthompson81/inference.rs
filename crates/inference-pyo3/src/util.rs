...
use inference_core::{
    AudioInput, ChatCompletionResponse, CompletionResponse, InferenceRs, Request, Response,
...
impl From<inference_core::InferenceRsError> for PyApiErr {
    fn from(value: inference_core::InferenceRsError) -> Self {
        Self::from(value.to_string())
    }
}
...
fn decode_video_ffmpeg(bytes: &[u8], source_hint: &str) -> anyhow::Result<VideoInput> {
    ...
    let tmp_dir = std::env::temp_dir().join("inference_video");
