...
template <typename DType, typename CacheType>
cudaError_t dispatch_flashinfer_decode_head_dim(const DecodeArgs &args,
                                                int32_t head_size) {
  switch (head_size) {
  case 64:
    return dispatch_flashinfer_decode_softcap<DType, CacheType, 64>(args);
  case 128:
    return dispatch_flashinfer_decode_softcap<DType, CacheType, 128>(args);
  case 256:
    return dispatch_flashinfer_decode_softcap<DType, CacheType, 256>(args);
  case 512:
    return dispatch_flashinfer_decode_softcap<DType, CacheType, 512>(args);
  default:
...
extern "C" void reshape_and_cache_flashinfer(
  ...
  if (dtype == 0 && cache_dtype == 0) {
    inference_flashinfer::reshape_and_cache_flashinfer_kernel<__half, __half>
        <<<grid, block, 0, stream>>>(
    ...
    inference_flashinfer::reshape_and_cache_flashinfer_kernel<__nv_bfloat16,
                                                               __nv_bfloat16>
    ...
    inference_flashinfer::reshape_and_cache_flashinfer_kernel<float, float>
        <<<grid, block, 0, stream>>>(
    ...
    inference_flashinfer::reshape_and_cache_flashinfer_kernel<__half,
                                                               __nv_fp8_e4m3>
        <<<grid, block, 0, stream>>>(
            static_cast<__half *>(key), static_cast<__half *>(value),
    ...
    inference_flashinfer::reshape_and_cache_flashinfer_kernel<float,
                                                               __nv_fp8_e4m3>
        <<<grid, block, 0, stream>>>(
            static_cast<float *>(key), static_cast<float *>(value),
            static_cast<__nv_fp8_e4m3 *>(key_cache),
...
extern "C" int32_t flashinfer_decode(
    ...
    uint32_t cache_dtype,
    cudaStream_t stream) {
  const inference_flashinfer::DecodeArgs args{
...
#define CALL_FLASHINFER_DECODE(DTYPE, CACHE_DTYPE)                             \
  return inference_flashinfer::dispatch_flashinfer_decode_head_dim<           \
      DTYPE, CACHE_DTYPE>(args, head_size)
  try {
    if (dtype == 0 && cache_dtype == 0) {
...
extern "C" void gather_kv_cache_flashinfer(
  ...
  if (out_dtype == 0 && cache_dtype == 0) {
    inference_flashinfer::gather_kv_cache_flashinfer_kernel<__half, __half>
        <<<grid, block, 0, stream>>>(
            static_cast<__half *>(key_cache),
    ...
    inference_flashinfer::gather_kv_cache_flashinfer_kernel<__nv_bfloat16,
                                                            __nv_bfloat16>
    ...
    inference_flashinfer::gather_kv_cache_flashinfer_kernel<float, float>
        <<<grid, block, 0, stream>>>(
            static_cast<float *>(key_cache), static_cast<float *>(value_cache),
    ...
    inference_flashinfer::gather_kv_cache_flashinfer_kernel<__nv_fp8_e4m3,
                                                            __half>
        <<<grid, block, 0, stream>>>(
            static_cast<__nv_fp8_e4m3 *>(key_cache),
