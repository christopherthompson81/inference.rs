...
//! refresh them with `cargo test -p inference-cli regenerate_cli_reference -- --ignored`.
...
const REGEN_HINT: &str = "cargo test -p inference-cli regenerate_cli_reference -- --ignored";
...
fn rendered_pages() -> Vec<(String, String)> {
    ...
    for (i, sub) in visible_subcommands(&root).enumerate() {
        let path = format!("inference {}", sub.get_name());
        pages.push((
